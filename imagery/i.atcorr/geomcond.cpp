...
/*
  To compute the solar azimuthal and zenithal angles (in degrees) for a point
  over the globe defined by its longitude and its latitude (in dec. degrees) for
  a day of the year (fixed by number of the month and number of the day in the
  month) at any Greenwich Meridian Time (GMT dec. hour).
*/
...
void GeomCond::pos_fft(long int j, double tu)
{
    ...
    caz = (-cos(xla) * sin(gdelta) + sin(xla) * cos(gdelta) * cos(ah)) /
          cos(elev);
    azim = asin(az);
    if (caz <= 0.f)
        azim = M_PI - azim;
...
void GeomCond::posobs(double tu, int nc, int nl)
{
    ...
    if ((1. / cosx2) > ((yk * yk) / (yk * yk - 1.))) {
        ...
        sn = (rs - (re * (sqrt((yk * yk) - (yk * yk - 1.) * (1. / cosx2))))) /
             (1. / cosx2);
        zt = rs - sn;
        xt = -(sn * tanx);
        yt = sn * tany / cos(x);
    ...
    if (igeom == 1)
        xlon = (double)(ylon * crd);
    else if (igeom == 2)
        xlon = (double)(ylon * crd - 75.);
    else
        xlon = (double)(ylon * crd - 135.);
    ...
    if (igeom == 1)
        ...
        ylon = xlon * M_PI / 180. + 75. * cdr;
    else
        ylon = xlon * M_PI / 180. + 135. * cdr;
...
void GeomCond::posnoa(double tu, int nc, double xlonan, double campm,
                      double hna)
{
    /*     noaa 6 definition
    ...
    if (nc != 1024) {
        ...
        double xden =
            (sin(zlat) - sin(ylat) * cos(d)) / cos(ylat) / sin(fabs(d));
...
void GeomCond::parse()
{
    ...
    default:
        G_fatal_error(_("Unsupported/unreadable format in control file (found "
                        "igeom=%ld)"),
                      igeom);
    }
    ...
    xmud = -xmus * xmuv - (double)sqrt(1.f - xmus * xmus) *
                              (double)sqrt(1.f - xmuv * xmuv) * xmup;
...
void GeomCond::print()
{
    static const string etiq1[] = {string(" user defined conditions     "),
