            ...
            InterpStruct &is)
{
    ...
    if (iaer != 0) {
        alphaa =
            (double)(log(sixs_aer.phase[lsup] / sixs_aer.phase[linf]) / coef);
    ...
    double d2 = 2 + delta;
    is.phar =
        (2 * (1 - delta) / d2) * .75f * (1 + xmud * xmud) + 3 * delta / d2;
    if (idatmp == 0) {
        betar = 0;
        betaa = 0;
        betac = 0;
        ...
        if (sixs_disc.roatm[0][linf] < 0.001) {
            is.rorayl = sixs_disc.roatm[0][linf] +
                        (sixs_disc.roatm[0][lsup] - sixs_disc.roatm[0][linf]) *
                            (wl - sixs_disc.wldis[linf]) /
                            (sixs_disc.wldis[lsup] - sixs_disc.wldis[linf]);
        }
        else {
            alphar = (double)(log(sixs_disc.roatm[0][lsup] /
                                  sixs_disc.roatm[0][linf]) /
                              coef);
        ...
        if (sixs_disc.roatm[1][linf] < 0.001) {
            is.romix = sixs_disc.roatm[1][linf] +
                       (sixs_disc.roatm[1][lsup] - sixs_disc.roatm[1][linf]) *
                           (wl - sixs_disc.wldis[linf]) /
                           (sixs_disc.wldis[lsup] - sixs_disc.wldis[linf]);
        }
        else {
            alphac = (double)(log(sixs_disc.roatm[1][lsup] /
                                  sixs_disc.roatm[1][linf]) /
                              coef);
        ...
        if (iaer != 0) {
            ...
            if (sixs_disc.roatm[2][linf] < 0.001) {
                is.roaero =
                    sixs_disc.roatm[2][linf] +
                    (sixs_disc.roatm[2][lsup] - sixs_disc.roatm[2][linf]) *
                        (wl - sixs_disc.wldis[linf]) /
                        (sixs_disc.wldis[lsup] - sixs_disc.wldis[linf]);
            }
            else {
                alphaa = (double)(log(sixs_disc.roatm[2][lsup] /
                                      sixs_disc.roatm[2][linf]) /
                                  coef);
    ...
    alphar =
        (double)(log(sixs_disc.trayl[lsup] / sixs_disc.trayl[linf]) / coef);
    betar = (double)(sixs_disc.trayl[linf] / pow(wlinf, alphar));
    is.tray = (double)(betar * pow(wl, alphar));

    if (idatmp != 0) {
        alphar = (double)(log(sixs_disc.traypl[lsup] / sixs_disc.traypl[linf]) /
                          coef);
    ...
    if (iaer != 0) {
        alphaa = (double)(log(sixs_aer.ext[lsup] * sixs_aer.ome[lsup] /
                              (sixs_aer.ext[linf] * sixs_aer.ome[linf])) /
                          coef);
        betaa = (double)(sixs_aer.ext[linf] * sixs_aer.ome[linf] /
                         pow(wlinf, alphaa));
        ...
        is.taerp =
            (double)(taer55p * betaa * pow(wl, alphaa) / sixs_aer.ext[3]);
