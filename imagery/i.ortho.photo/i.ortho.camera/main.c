...
int main(int argc, char *argv[])
{
    ...
    if (G_legal_filename(camera) < 0)
        G_fatal_error(_("<%s> is an illegal file name"), camera);

    if (cam_name || cam_id || cfl_opt->answer || pp_opt->answers ||
        fid_opt->answers)
        put_cam_info = 1;
