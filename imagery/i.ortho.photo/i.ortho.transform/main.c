...
static void compute_transformation(void)
{
    ...
    for (n = 0; n < points->count; n++) {
        ...
        if (need_fwd) {
            ...
            I_georef(points->e1[n], points->n1[n], &etmp, &ntmp, group.E12,
                     group.N12, 1);
...
static void dump_cooefs(void)
{
    ...
    for (i = 0; i < 3; i++)
        fprintf(stdout, "E%d=%.15g\n", i,
                forward ? group.E12[i] : group.E21[i]);

    for (i = 0; i < 3; i++)
        fprintf(stdout, "N%d=%.15g\n", i,
                forward ? group.N12[i] : group.N21[i]);
...
int main(int argc, char **argv)
{
    ...
    rev_flag->description =
        _("Target east,north,height coordinates to local x,y,z");
