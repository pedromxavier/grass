...
int main(int argc, char *argv[])
{
    ...
    output->key = "output";
    output->description = _("Name for output QC type classification layer");
    ...
    G_asprintf(
        ...
        "mcd43B2q;%s;"
        "mod13Q1;%s;",
        _("surf. refl. 250m 8-days"), _("surf. refl. 500m 8-days"),
        _("surf. refl. 500m 8-days, State QA"), _("surf. refl. 500m daily"),
        _("surf. refl. 500m daily, State QA"), _("surf. refl. 5000m daily"),
        _("surf. refl. 5000m daily, State QA"),
        _("surf. refl. 5000m daily, Internal Climatology"),
        _("LST 1km daily (Day/Night)"), _("LST 1km 8-days (Day/Night)"),
        _("VI 1km 16-days"),
        _("Brdf-Albedo Quality (Ancillary SDS) 1km 8-days"),
        _("Brdf-Albedo Quality (BRDF SDS) 1km 8-days"), _("VI 250m 16-days"));
    productname->descriptions = desc_productname;
    productname->options =
        "mod09Q1,mod09A1,mod09A1s,mod09GA,mod09GAs,mod09CMG,mod09CMGs,"
        "mod09CMGi,mod11A1,mod11A2,mod13A2,mcd43B2,mcd43B2q,mod13Q1";
    ...
    G_asprintf(
        ...
        "possible_snow_ice;%s;"
        "possible_shadow;%s;",
        _("mod09: Adjacency Correction"), _("mod09: Atmospheric Correction"),
        _("mod09: Cloud State"), _("mod09: Band-Wise Data Quality Flag"),
        ...
        _("mod09*s: StateQA Cloud Shadow"), _("mod09*s: StateQA Cloud State"),
        _("mod09*s: StateQA Internal Cloud Algorithm"),
        _("mod09*s: StateQA Internal Fire Algorithm"),
        _("mod09*s: StateQA Internal Snow Mask"),
        _("mod09*s: StateQA Land Water"), _("mod09*s: StateQA mod35 Snow Ice"),
        _("mod09*s: StateQA Pixel Adjacent to Cloud"),
        _("mod09*s: StateQA Salt Pan (mod09GAs)"),
        _("mod09*i: Internal CM: Cloudy"), _("mod09*i: Internal CM: Clear"),
        _("mod09*i: Internal CM: High Clouds"),
        _("mod09*i: Internal CM: Low Clouds"), _("mod09*i: Internal CM: Snow"),
        _("mod09*i: Internal CM: Fire"), _("mod09*i: Internal CM: Sun Glint"),
        _("mod09*i: Internal CM: Dust"),
        _("mod09*i: Internal CM: Cloud Shadow"),
        _("mod09*i: Internal CM: Pixel is Adjacent to Cloud"),
        _("mod09*i: Internal CM: Cirrus"), _("mod09*i: Internal CM: Pan Flag"),
    ...
    qcname->options =
        "adjcorr,atcorr,cloud,data_quality,diff_orbit_from_500m,modland_qa,"
        "mandatory_qa_11A1,data_quality_flag_11A1,emis_error_11A1,lst_error_"
        "11A1,data_quality_flag_11A2,emis_error_11A2,mandatory_qa_11A2,lst_"
        "error_11A2,aerosol_quantity,brdf_correction_performed,cirrus_detected,"
        "cloud_shadow,cloud_state,internal_cloud_algorithm,internal_fire_"
        "algorithm,internal_snow_mask,land_water,mod35_snow_ice,pixel_adjacent_"
        "to_cloud,salt_pan,icm_cloudy,icm_clear,icm_high_clouds,icm_low_clouds,"
        "icm_snow,icm_fire,icm_sun_glint,icm_dust,icm_cloud_shadow,icm_pixel_"
        "is_adjacent_to_cloud,icm_cirrus,icm_pan_flag,icm_criteria_for_aerosol_"
        "retrieval,icm_aot_has_clim_val,modland_qa,vi_usefulness,aerosol_"
        "quantity,pixel_adjacent_to_cloud,brdf_correction_performed,mixed_"
        "clouds,land_water,possible_snow_ice,possible_shadow,platform,land_"
        "water,sun_z_angle_at_local_noon,brdf_correction_performed,modland_qa,"
        "vi_usefulness,aerosol_quantity,pixel_adjacent_to_cloud,brdf_"
        "correction_performed,mixed_clouds,land_water,possible_snow_ice,"
        "possible_shadow";
    ...
    input_band->description =
        _("Band number of MODIS product "
          "(mod09Q1=[1,2],mod09A1=[1-7],m[o/y]d09GA=[1-7],m[o/y]d09CMG=[1-7], "
          "mcd43B2q=[1-7])");
    desc_input_band = NULL;
    G_asprintf(&desc_input_band, "1;%s;2;%s;3;%s;4;%s;5;%s;6;%s;7;%s",
               _("Band 1: Red"), _("Band 2: NIR"), _("Band 3: Blue"),
               _("Band 4: Green"), _("Band 5: SWIR 1"), _("Band 6: SWIR 2"),
               _("Band 7: SWIR 3"));
    ...
    /*mod09Q1 */
    if (strcmp(product, "mod09Q1") /*if not mod09Q1 but its qcflag was entered,
                                      issue error */
        &&
        (!strcmp(qcflag, "cloud") || !strcmp(qcflag, "diff_orbit_from_500m")))
        G_fatal_error(
            _("This bit flag is only available for MOD09Q1 @ 250m products"));

    if (!strcmp(qcflag, "data_quality")) { /* verify the allowed number of bands
                                              for each product */
    ...
    if ((strcmp(qcflag, "BRDF_correction_performed") &&
         (!(strcmp(product, "mod09A1s")) || !(strcmp(product, "mod09CMGs")))))
        G_fatal_error(_("This bit flag is only available for MOD09A1s @ 500m "
                        "or MOD09CMG @ 5000m products"));

    /*mod09GA stateqa */
    if ((!strcmp(qcflag, "salt_pan") && strcmp(product, "mod09GAs")))
        G_fatal_error(
            _("This bit flag is only available for MOD09GAs @ 500m products"));

    /*mod09CMG */
    if (strcmp(product, "mod09CMGi") /*if not mod09CGMi but its qcflag was
                                        entered, issue error */
        &&
        (!strcmp(qcflag, "icm_cloudy") || !strcmp(qcflag, "icm_clear") ||
         !strcmp(qcflag, "icm_high_clouds") ||
         !strcmp(qcflag, "icm_low_clouds") || !strcmp(qcflag, "icm_snow") ||
         !strcmp(qcflag, "icm_fire") || !strcmp(qcflag, "icm_sun_glint") ||
         !strcmp(qcflag, "icm_dust") || !strcmp(qcflag, "icm_cloud_shadow") ||
         !strcmp(qcflag, "icm_pixel_is_adjacent_to_cloud") ||
         !strcmp(qcflag, "icm_cirrus") || !strcmp(qcflag, "icm_pan_flag") ||
         !strcmp(qcflag, "icm_criteria_for_aerosol_retrieval") ||
         !strcmp(qcflag, "icm_aot_has_clim_val")))
        G_fatal_error(_(
            "This bit flag is only available for mod09CMGi @ 5000m products"));

    /*mod13A2 */
    if (strcmp(product, "mod13A2") /*if not mod13A2 but its qcflag was entered,
                                      issue error */
        &&
        (!strcmp(qcflag, "vi_usefulness") || !strcmp(qcflag, "mixed_clouds") ||
         !strcmp(qcflag, "possible_snow_ice") ||
         !strcmp(qcflag, "possible_shadow")))
        G_fatal_error(
            _("This bit flag is only available for MOD13A2 @ 1km products"));

    /*mcd43B2 */
    if (strcmp(product, "mcd43B2") /*if not mcd43B2 but its qcflag was entered,
                                      issue error */
        && (!strcmp(qcflag, "platform") || !strcmp(qcflag, "land_water") ||
            !strcmp(qcflag, "sun_z_angle_at_local_noon")))
        G_fatal_error(
            _("This bit flag is only available for MCD43B2 @ 1km products"));
    ...
    /*mod13Q1 */
    if (strcmp(product, "mod13Q1") /*if not mod13Q1 but its qcflag was entered,
                                      issue error */
        &&
        (!strcmp(qcflag, "vi_usefulness") || !strcmp(qcflag, "mixed_clouds") ||
         !strcmp(qcflag, "possible_snow_ice") ||
         !strcmp(qcflag, "possible_shadow")))
        G_fatal_error(
            _("This bit flag is only available for MOD13Q1 @ 250m products"));
    ...
    for (row = 0; row < nrows; row++) {
        ...
        for (col = 0; col < ncols; col++) {
            ...
            if (Rast_is_c_null_value(&c))
                ...
                G_fatal_error(_(
                    "Unknown names and/or combination, please check spelling"));
