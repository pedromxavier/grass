...
/* future implementation
   double get_metdouble(const char metadata[], char *format, int code, char
   value[])
   {
   char key[MAX_STR];
...
/* future implementation
   double get_mtldouble(const char metadata[], char *format, int code, char
   value[])
   {
   char key[MAX_STR];
...
void lsat_metadata(char *metafile, lsat_data *lsat)
{
    ...
    if (value[0] != '\0') {
        ...
        G_warning("Using production date from the command line 'product_date'");
    ...
    if (get_mtldata == get_mtlformat) {
        if (ver_mtl == 0) {
            ...
            get_mtldata(mtldata, "EARTH_SUN_DISTANCE",
                        value); /* Necessary after */
            if (value[0] != '\0')
            ...
            if (strstr(mtldata, "RADIANCE_MAXIMUM_BAND") != NULL) {
                ...
                for (i = 0; i < lsat->bands; i++) {
                    ...
                    lsat->band[i].lmax =
                        ...
                        lsat->band[i].bias;
                    /* ... qcalmax and qcalmin loaded in previous sensor_
                     * function */
                    if (lsat->number == 8) {
                        if (lsat->band[i].thermal) {
                            ...
                            lsat->band[i].esun =
                                ...
                                lsat->band[i].K2;
                            /*
                               double esun1 = (double) (PI * lsat->dist_es *
                               lsat->dist_es * lsat->band[i].bias) /
                               lsat->band[i].K2; double esun2 = (double) (PI *
                               lsat->dist_es * lsat->dist_es *
                               lsat->band[i].gain) / lsat->band[i].K1;
                ...
                G_warning(
                    "ESUN evaluated from REFLECTANCE_ADDITIVE_FACTOR_BAND");
            }
        }
    }
    else {
        ...
        for (i = 0; i < lsat->bands; i++) {
            ...
            lsat->band[i].lmin =
                lsat->band[i].gain * lsat->band[i].qcalmin + lsat->band[i].bias;
            lsat->band[i].lmax =
                lsat->band[i].gain * lsat->band[i].qcalmax + lsat->band[i].bias;
        }
    }
