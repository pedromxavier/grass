...
int mean_shift(struct globals *globals)
{
    ...
    G_message(_("Estimating spectral bandwidth for spatial bandwidth %g..."),
              hspat);
    G_percent_reset();
    for (row = globals->row_min; row < globals->row_max; row++) {
    ...
    G_debug(1, "Average minimum difference excl zero to neighbours: %g",
            mindiffzeroavg);
            ...
            *pngbr_rc = ngbr_rc;
            ...
            if (!no_check && pavl_insert(visited, pngbr_rc) == NULL) {
                ...
                if (ngbr_id == this_id) {
                    ...
                    Segment_get(globals->bands_out, (void *)Rk.mean, rown,
                                coln);
