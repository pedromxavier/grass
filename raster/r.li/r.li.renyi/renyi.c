 ...
 *                Serena Pallecchi student of Computer Science
...
int main(int argc, char *argv[])
{
    ...
    if (atof(alpha->answer) == 1) {
        G_fatal_error("If alpha = 1 Renyi index is not defined. (Ricotta et "
                      "al., 2003, Environ. Model. Softw.)");
        exit(RLI_ERRORE);
    }
    else if (atof(alpha->answer) < 0.) {
        G_fatal_error("Alpha must be > 0 otherwise Renyi index is not defined. "
                      "(Ricotta et al., 2003, Environ. Model. Softw.)");
        exit(RLI_ERRORE);
    }
    else {
        par = &alpha->answer;
    ...
    if (area > 0) {
        ...
        if ((alpha < 1 && sum < 1) || (alpha > 1 && sum > 1)) {
            G_warning("Renyi index calculation reached numerical instability. "
                      "This can happen with alpha close to 1. The result will "
                      "be set to zero.");
            sum = 1;
        }
