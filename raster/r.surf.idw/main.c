...
int main(int argc, char **argv)
{
    ...
    G_add_keyword(_("IDW"));
    module->description = _("Provides surface interpolation from raster point "
    ...
    if ((maskfd = Rast_maskfd()) >= 0 ||
        error_flag) {   /* apply mask to output */
                ...
                int npoints, int out_fd, int maskfd)
{
    ...
    for (row = 0; row < nrows; row++) { /*  loop over rows      */
        ...
        for (col = 0; col < ncols; col++) { /*  loop over columns   */
            ...
            nbr_head->next = NULL;
            if (make_neighbors_list(
                    search, lastrow, current_row, row, col, nbr_head,
                    npoints)) { /* otherwise, known data value assigned */
