...
int main(int argc, char *argv[])
{
    ...
    parm.offset->description =
        _("If 0, no offset is added and the first band is 1");
    ...
    parm.map_names_file->description =
        _("Name of the output file that contains the imported map names");
    ...
    parm.rat->description =
        _("The band number and \".csv\" will be appended to the file prefix");
    ...
    flag_j->key = 'j';
    flag_j->description = _("Perform projection check only and exit");
    ...
    if (croptoregion) {
        ...
        G_debug(1, "first row in cur wind %d, first row in source %d", first,
                rowmapall[first]);
        ...
        G_debug(1, "first col in cur wind %d, first col in source %d", first,
                colmapall[first]);
                    ...
                    if (authkey) {
                        ...
                        if (authname && *authname) {
                            ...
                            if (authcode && *authcode) {
                                G_asprintf(&gdalsrid, "%s:%s", authname,
                                           authcode);
                            }
                        }
                    }
    ...
    if (complex) { /* CEOS SAR et al.: import flipped to match GRASS coordinates
        ...
        for (row = 0; row < nrows; row++) {
            ...
            if (GDALRasterIO(hBand, GF_Read, 0, rowmap[row], ncols_gdal, 1,
                             bufComplex, ncols_gdal, 1, eGDT, 0,
                             0) != CE_None) {
