...
static long int validate(struct posdata *pdat)
{
    ...
    if (pdat->function & L_GEOM) {
        if ((pdat->year < 1950) ||
            (pdat->year > 2050)) /* limits of algorithm */
...
static void geometry(struct posdata *pdat)
{
    ...
    if (pdat->time_updated) {
        ...
        if (pdat->ectime != 0) {
            ...
            pdat->ecobli = 84381.448 - 46.84024 * ectime - 5.9e-5 * ectime2 +
                           1.813e-3 * ectime3;
            /* convert to degrees */
            pdat->ecobli /= 3600.0;
...
static void tst(struct posdata *pdat)
{
    ...
    pdat->tstfix =
        pdat->tst - (float)pdat->hour * 60.0 - pdat->minute -
...
static void amass(struct posdata *pdat)
{
    if (pdat->zenref > 93.0) {
        ...
        pdat->amass = 1.0 / (cos(DEG2RAD * pdat->zenref) +
...
void S_decode(long code, struct posdata *pdat)
{
    ...
    if (code & (1L << S_DAY_ERROR))
        G_warning(_("S_decode ==> Please fix the day-of-month: %d"), pdat->day);
    ...
    if (code & (1L << S_TEMP_ERROR))
        G_warning(_("S_decode ==> Please fix the temperature: %f"), pdat->temp);
