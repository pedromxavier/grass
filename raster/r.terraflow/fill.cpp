...
void mergeStreamGridGrid(AMI_STREAM<T1> *grid1, AMI_STREAM<T2> *grid2,
                         dimension_type rows, dimension_type cols,
                         AMI_STREAM<T3> *str, FUN fo,
                      ...
                      AMI_STREAM<labelElevType> *&labeledWater)
{
    ...
    rt_start(rtTotal);
    assert(elstr && filledstr == NULL && dirstr == NULL &&
           labeledWater == NULL);
    if (stats) {
        stats->comment("------------------------------");
    ...
    FILL_SAVEALL
    {
        ...
        printStream2Grid(tmp, nrows, ncols, verbosedir("label1.asc"),
    ...
    */
    ...
    delete waterWindows;
    /* cout << "bogus boundary length = " << boundaryStr->stream_len() <<
     * endl;*/
    ...
    printStream2Grid(labeledWater, nrows, ncols, "watershed.asc",
                     labelElevType::printLabel);
#endif
    ...
    dirstr = new AMI_STREAM<direction_type>();
    statstr = new AMI_STREAM<plateauStats>();
    platstr = findPlateaus(filledstr, nrows, ncols,
                           nodataType::ELEVATION_NODATA, dirstr, statstr);
    rt_stop(rt);
    if (stats) {
    ...
    FILL_SAVEALL
    {
        FILL_DEBUG cout << "sort plateauStr (by ij): ";
        AMI_STREAM<plateauType> *tmp = sort(platstr, ijCmpPlateauType());
        printStream2Grid(tmp, nrows, ncols, verbosedir("plateaus.asc"),
                         plateauType::printLabel);
        delete tmp;
    }
    ...
    printGridStream(filledstr, nrows, ncols, "filled_elev.asc",
                    printElevation());
    printGridStream(flowStream, nrows, ncols, "direction.asc",
                    printDirection());
#endif
...
public:
    ...
    waterGridType operator()(elevation_type el, direction_type dir)
    {
        ...
        assert(!is_nodata(el) || (wg.getLabel() == LABEL_BOUNDARY ||
                                  wg.getLabel() == LABEL_UNDEF));
        return wg;
    }
};
                ...
                AMI_STREAM<elevation_type> *elStr)
{
    ...
    mergeStreamGridGrid(elStr, dirStr, nrows, ncols, waterStr,
                        directionElevationMerger(), mergedWaterStr);
    delete waterStr;
                     ...
                     AMI_STREAM<waterWindowBaseType> *merge)
{
    ...
    waterStr = sort(unsortedWaterStr, ijCmpWaterType());
    mergeStreamGridGrid(elStr, dirStr, nrows, ncols, waterStr,
                        directionElevationMerger(), merge);
    delete waterStr;
}
...
void mergeStreamGridGrid(AMI_STREAM<T1> *grid1, AMI_STREAM<T2> *grid2,
                         dimension_type rows, dimension_type cols,
                         AMI_STREAM<T3> *str, FUN fo, AMI_STREAM<T4> *outStream)
{
    T1 *t1p;
    T2 *t2p;
    T3 *t3p;
...
findBoundariesMain(AMI_STREAM<labelElevType> *labeledWater)
{
    ...
    FILL_SAVEALL
    {
        ...
        printStream2Grid(boundaryStr, nrows, ncols, verbosedir("boundary.asc"),
                         boundaryType::print);
    }
