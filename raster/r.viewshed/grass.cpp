 ...
 */
surface_type adjust_for_curvature(Viewpoint vp, double row, double col,
                                  surface_type h, ViewOptions viewOptions,
                                  GridHeader *hd)
{
...
GridHeader *read_header(char *rastName, Cell_head *region)
{
    ...
    if (nrows <= maxDimension && ncols <= maxDimension) {
        ...
        G_warning("ERROR: nrows (%d) > maxDimension (%d) AND/OR ncols (%d) > "
                  "maxDimension (%d)",
                  nrows, maxDimension, ncols, maxDimension);
        G_fatal_error(_("Computational region too large. Use smaller area or "
                                 ...
                                 MemoryVisibilityGrid *visgrid)
{
    ...
    *data = (surface_type **)G_malloc(3 * sizeof(surface_type *));
    assert(*data);
    (*data)[0] =
        (surface_type *)G_malloc(3 * Rast_window_cols() * sizeof(surface_type));
    assert((*data)[0]);
    ...
    for (i = 0; i < (dimensionType)nrows; i++) {
        ...
        for (j = 0; j < (dimensionType)ncols; j++) {
            ...
            e.elev[1] =
                adjust_for_curvature(*vp, i, j, e.elev[1], viewOptions, hd);
            ...
            e.elev[0] = calculate_event_elevation(e, nrows, ncols, vp->row,
                                                  vp->col, inrast, data_type);
            /* adjust for curvature */
            if (viewOptions.doCurv) {
                calculate_event_position(e, vp->row, vp->col, &ay, &ax);
                e.elev[0] = adjust_for_curvature(*vp, ay, ax, e.elev[0],
                                                 viewOptions, hd);
            }
            ...
            if (viewOptions.doCurv) {
                calculate_event_position(e, vp->row, vp->col, &ay, &ax);
                e.elev[2] = adjust_for_curvature(*vp, ay, ax, e.elev[2],
                                                 viewOptions, hd);
            }
                                    ...
                                    IOVisibilityGrid *visgrid)
{
    ...
    if (data != NULL) {
        ...
        *data = (surface_type **)G_malloc(3 * sizeof(surface_type *));
        assert(*data);
        (*data)[0] = (surface_type *)G_malloc(3 * Rast_window_cols() *
                                              sizeof(surface_type));
        assert((*data)[0]);
        (*data)[1] = (*data)[0] + Rast_window_cols();
        (*data)[2] = (*data)[1] + Rast_window_cols();
    ...
    for (i = 0; i < (dimensionType)nrows; i++) {
        ...
        for (j = 0; j < (dimensionType)ncols; j++) {
            if (is_invisible_nodata(grid->grid_data[i][j])) {
                ...
                ((CELL *)outrast)[j] =
                    (CELL)booleanVisibilityOutput(grid->grid_data[i][j]);
            }
            else if (mode == OUTPUT_ANGLE) {
                if (is_visible(grid->grid_data[i][j])) {
                    ((FCELL *)outrast)[j] =
                        (FCELL)angleVisibilityOutput(grid->grid_data[i][j]);
                }
                else {
...
void save_io_visibilitygrid_to_GRASS(IOVisibilityGrid *visgrid, char *fname,
