...
int get_item(FILE *fd, int format, int *type, int *cat_int, double *cat_double,
             double **x, double **y, int *count, struct Categories *labels)
{
    ...
    *type = 0;
    ...
    while (1) {
        ...
        /* if we've found the next feature, rewind to the start of it and
         * complete */
        ...
        /* if we found a cat (and optionally a label), read them and continue to
         * scan */
        if (*buf == '=') {
            if (format == USE_FCELL || format == USE_DCELL) {
                ...
                if (sscanf(buf + 1, "%lf%[^\n]", cat_double, lbl) == 2) {
                    ...
                    if (format == USE_FCELL) {
                        ...
                        Rast_set_f_cat(&cat_float_tmp, &cat_float_tmp, lbl,
                                       labels);
                    }
                    else {
                        Rast_set_d_cat((DCELL *)cat_double, (DCELL *)cat_double,
                                       lbl, labels);
                    }
                }
                continue;
                ...
                if (sscanf(buf + 1, "%d%[^\n]", cat_int, lbl) == 2) {
                    G_strip(lbl);
                    Rast_set_c_cat((CELL *)cat_int, (CELL *)cat_int, lbl,
                                   labels);
                }
                continue;
            }
