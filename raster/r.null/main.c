...
int main(int argc, char *argv[])
{
    ...
    parms.map->description =
        _("Name of raster map for which to edit null values");
    ...
    if (only_null && G_find_file2_misc("cell_misc", "null", name, mapset))
        G_fatal_error(_("Raster map <%s> already has a null bitmap file"),
                      name);

    if (map_type == CELL_TYPE) {
        if (only_fp)
            G_fatal_error(_("<%s> is integer raster map (CELL)"), name);
        ...
        G_fatal_error(_("<%s> is floating pointing raster map"), name);
    ...
    if (recreate) {
        ...
        G_debug(1, "NULL compression is currently %s",
                donullcompr ? "enabled" : "disabled");
...
int process(const char *name, const char *mapset, int change_null,
            RASTER_MAP_TYPE map_type)
{
    struct Colors colr;
    struct History hist;
...
int doit(const char *name, const char *mapset, int change_null,
         RASTER_MAP_TYPE map_type)
{
    int new, old, row;
    void *rast;
