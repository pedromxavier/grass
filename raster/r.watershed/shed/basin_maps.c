...
int basin_maps(INPUT *input, OUTPUT *output)
{
    ...
    G_message(_("parameter hydrologic/soil erosion model.  Continue inputing "
                "cell map"));
    ...
    while (mapset != NULL) {
        ...
        if (output->num_maps == 1)
            ...
            output->maps =
                (MAP *)G_realloc(output->maps, output->num_maps * sizeof(MAP));
    ...
    G_message(_("\nThe output from %s will be divided into watershed"),
              G_program_name());
    G_message(_("basins.  There are two possible methods of tabulating the "
                "information:"));
    G_message(_("1) by only including data pertaining to the basin itself, or "
                "2) using"));
    ...
    if (input->fast) {
        G_message(_("\nOK, %s should start running now using the "
                    "following form:\n%s"),
                  RAM_NAME, input->com_line_ram);
    }
    else {
        G_message(_("\nOK, %s should start running now using the "
                    "following form:\n%s"),
                  SEG_NAME, input->com_line_seg);
    }
