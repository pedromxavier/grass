                        ...
                        int orthorot, char *sep, FILE *fp)
{
    ...
    if (use3d)
        ...
        fprintf(fp,
                "index%sfwd_dx%sfwd_dy%sback_dx%sback_dy%sfwd_RMS%sback_RMS",
                       ...
                       int orthorot, int rms, char *sep, FILE *fpr)
{
    ...
    if (use3d) {
        ...
        fclose(fp);
        if (stat < 0) {
            G_fatal_error(_("Bad format in control point file <%s>"), pfile);
            return 0;
        }
    ...
    case -2:
        strcat(msg,
               _("Not enough memory to solve for transformation equation"));
        break;
    case -3:
