...
/* output reclass cats */
static void extract_cats(struct line_cats *Cats, int type_only, int field,
                         int new, int reverse)
{
    int i, tmp;
                                ...
                                dbDriver *driver, struct field_info *Fi)
{
    ...
    for (i = 0; i < Cats1->n_cats; i++) {
        ...
        if (dissolve_key && Cats1->field[i] == field) {
            db_select_value(driver, Fi->table, Fi->key, Cats1->cat[i],
                            dissolve_key, &val1);
        }
        for (j = 0; j < Cats2->n_cats; j++) {
            ...
            if (dissolve_key) {
                db_select_value(driver, Fi->table, Fi->key, Cats2->cat[j],
                                dissolve_key, &val2);
                /* compare db values */
                switch (coltype) {
                case DB_C_TYPE_INT: {
                 ...
                 char *dissolve_key, int field, int type_only, int reverse)
{
    ...
    for (line = 1; line <= nlines; line++) {
        ...
        if (field > 0)
            field_match =
                Vect_cat_get(Line_Cats_Old, field, &tmp) > 0 ? TRUE : FALSE;
        ...
        if (type == GV_BOUNDARY) {
            ...
            if (left_area > 0) {
                ...
                if (centroid > 0) {
                    ...
                    left_field_match =
                        Vect_cat_get(CCats, field, &tmp) > 0 ? TRUE : FALSE;
