...
int main(int argc, char *argv[])
{
    ...
    if (n_max_steps > 1) {
        ...
        for (curr_step = 1; curr_step < n_max_steps - 1; curr_step++) {
            ...
            max_step[curr_step] =
                max_map / (2 << (n_max_steps - 1 - curr_step));
        }
    ...
    if (update_table) {
        ...
        while (opt.column->answers[i]) {
            db_get_column(driver, Fi->table, opt.column->answers[i], &column);
            if (column) {
            ...
            while (!done) {
                ...
                for (i = 0; i < aList->n_values; i++) {
                    ...
                    if (poly == 3) {
                        ...
                        if (poly == 1) {
                            double tmp2_tx, tmp2_ty, tmp2_tz, tmp2_talong,
                                tmp2_tangle;
                            double tmp2_fx, tmp2_fy, tmp2_fz, tmp2_falong,
                                tmp2_fangle;
                            double tmp2_dist;
