...
void add_point(const int cat, const double dist, const double z,
               size_t *rescount, const int open3d)
{
    ...
    if (*rescount + 1 > alloc_size) {
        ...
        tmp =
            (Result *)G_realloc(resultset, sizeof(Result) * (*rescount + 100));
...
/* Check if point is on profile line (inside buffer) and calculate
 * distance to it */
void proc_point(struct line_pnts *Points, struct line_pnts *Profil,
                struct line_pnts *Buffer, const int cat, size_t *rescount,
                const int open3d)
{
    double dist;

    if (Vect_point_in_poly(*Points->x, *Points->y, Buffer) > 0) {
        Vect_line_distance(Profil, *Points->x, *Points->y, *Points->z, open3d,
               ...
               const int cat, size_t *rescount, const int open3d)
{
    ...
    for (i = 0; i < Ipoints->n_points; i++) {
        Vect_line_distance(Profil, Ipoints->x[i], Ipoints->y[i], Ipoints->z[i],
