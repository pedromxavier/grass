...
static int write_line(struct grid_description *grid_info, struct Map_info *Map,
                      struct line_pnts *Points, struct line_cats *Cats,
                      int *cat, int type, double x1, double y1, double x2,
                      double y2, int nbreaks)
{
    int i;
    double x, y;
    double dx, dy;
        ...
        if (row & 1) {
            if (otype & GV_POINTS) {
                for (col = 1; col < cols; col += 2) {
                    east = grid_info->west + grid_info->cstep * col +
                           grid_info->crad;
                    x1 = east;
                    y1 = north;
            ...
            if ((col & 1) && (otype & GV_LINES)) {
                ...
                write_line(grid_info, Map, Points, Cats, &cat, ltype, x1, y1,
                           x2, y2, nbreaks);
            ...
            for (col = 0; col < cols; col += 2) {
                east =
                    grid_info->west + grid_info->cstep * col + grid_info->crad;
                ...
                if (otype & GV_LINES) {
                    ...
                    write_line(grid_info, Map, Points, Cats, &cat, ltype, x1,
                               y1, x2, y2, nbreaks);
