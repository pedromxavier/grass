 ...
 *
 * MODULE:       v.mkgrid
 * AUTHOR(S):    Written by GRASS, Fall of 88,
 *                 Michael Higgins CERL (original contributor)
 *
 *               Updated <02 Jun 1992> by Darrell McCauley
 *                 <mccauley@ecn.purdue.edu> angle option added.
...
int main(int argc, char *argv[])
{
    ...
    desc = NULL;
    G_asprintf(&desc, "region;%s;coor;%s", _("current region"),
    ...
    breaks->required = NO;
    breaks->description = _("Number of vertex points per grid cell");
    breaks->options = "0-60";
    breaks->answer = "0";
    ...
    hex_flag->key = 'h';
    hex_flag->description = _("Create hexagons (default: rectangles)");

    ha_flag = G_define_flag();
    ha_flag->key = 'a';
    ha_flag->description = _("Allow asymmetric hexagons");

    diag_flag = G_define_flag();
    diag_flag->key = 'd';
    diag_flag->label = _("EXPERIMENTAL: Add diagonals to rectangular lines");
    diag_flag->description = _("Applies only to lines for rectangles");
    ...
    if (position_opt->answer[0] == 'r') { /* region */
        if (coord->answer)
            G_fatal_error(
                _("'coor' and 'position=region' are exclusive options"));

        if (box->answer && grid->answer)
            G_fatal_error(_("'box' and 'grid' are exclusive options for "
                            "'position=region'"));
        ...
        if (grid->answer) {
            ...
            grid_info.width =
                (grid_info.east - grid_info.west) / grid_info.num_cols;
            grid_info.height =
                (grid_info.north - grid_info.south) / grid_info.num_rows;
        }
        else if (box->answer) {
            if (!G_scan_resolution(box->answers[0], &(grid_info.width),
                                   window.proj))
                G_fatal_error(_("Invalid width"));
            if (!G_scan_resolution(box->answers[1], &(grid_info.height),
                                   window.proj))
                G_fatal_error(_("Invalid height"));

            /* register to lower left corner as for position=coor */
            grid_info.num_cols =
                (grid_info.east - grid_info.west + grid_info.width / 2.0) /
                grid_info.width;
            grid_info.num_rows =
                (grid_info.north - grid_info.south + grid_info.height / 2.0) /
                grid_info.height;
            grid_info.north =
                grid_info.south + grid_info.num_rows * grid_info.height;
            grid_info.east =
                grid_info.west + grid_info.num_cols * grid_info.width;
        }
        ...
        if (!G_scan_easting(coord->answers[0], &(grid_info.west), window.proj))
            G_fatal_error(_("Invalid easting"));
        if (!G_scan_northing(coord->answers[1], &(grid_info.south),
                             window.proj))
        ...
        grid_info.north =
            grid_info.south + grid_info.height * grid_info.num_rows;
    }
    ...
    if (hex_flag->answer) {
        ...
        grid_info.rstep =
            (grid_info.north - grid_info.south) / (grid_info.num_rows * 2);
        ...
        grid_info.num_vect_rows =
            (grid_info.north - grid_info.south) / grid_info.rstep;
        if (grid_info.north - grid_info.rstep * (grid_info.num_vect_rows + 1) <
            grid_info.south)
            grid_info.num_vect_rows--;
        grid_info.num_vect_cols =
            (grid_info.east - grid_info.west - grid_info.crad * 0.5) /
            grid_info.cstep;

        if (grid_info.east - grid_info.west < 3.5 * grid_info.crad) {
            G_fatal_error(
                _("Please use a higher resolution or a larger region"));
        }
        ...
        if ((int)(grid_info.num_vect_rows / 2.0 + 0.5) != grid_info.num_rows)
            G_message(_("The number of rows has been adjusted from %d to %d"),
                      grid_info.num_rows,
                      (int)(grid_info.num_vect_rows / 2.0 + 0.5));
