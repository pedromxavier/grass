...
int edge_detection(struct Cell_head elaboration_reg,
                   struct bound_box Overlap_Box, double *parBilin, double obsX,
                   double obsY, double *partial, double alpha, double residual,
                   double gradHigh, double gradLow)
{
    ...
    if ((gradPto > gradHigh) && (residual > 0))
    ...
    else if ((gradPto > gradLow) &&
             (residual > 0)) { /* Soft condition for 'edge' points */
        ...
        if (Vect_point_in_box(obsX, obsY, 0.0, &Overlap_Box)) {
            ...
            if ((fabs(atan(g[2][1] / g[2][0]) + M_PI / 2 - dirPto) < alpha) &&
                ...
                Get_Gradient(Elaboration, obsX + stepE * cos(dirPto + M_PI / 4),
                             obsY + stepN * sin(dirPto + M_PI / 4), parBilin,
                             gradient);
                g[1][0] = gradient[0];
                g[1][1] = gradient[1];
                ...
                Get_Gradient(Elaboration, obsX + stepE * cos(dirPto + M_PI / 2),
                             obsY + stepN * sin(dirPto + M_PI / 2), parBilin,
                             gradient);
                g[4][0] = gradient[0];
                g[4][1] = gradient[1];
                ...
                c2 = 0;
                for (c1 = 0; c1 < 9; c1++)
                    if (g[c1][0] * g[c1][0] + g[c1][1] * g[c1][1] > gradHigh)
                        c2++;
...
void classification(struct Map_info *Out, struct Cell_head Elaboration,
                    ...
                    double **obs, double *parBilin, double *parBicub,
                    double mean, double alpha, double gradHigh, double gradLow,
        ...
        if (Vect_point_in_box(obs[i][0], obs[i][1], mean, &General)) {
            ...
            *point->z += mean;
            ...
            if (Vect_point_in_box(obs[i][0], obs[i][1], interpolation,
                ...
                edge = edge_detection(Elaboration, Overlap, parBilin, *point->x,
                ...
                if ((*point->x > Overlap.E) && (*point->x < General.E)) {
                    if ((*point->y > Overlap.N) &&
                        ...
                        if (Select(&gradient[0], &gradient[1], &interpolation,
                                   line_num[i], driver, tab_name) != DB_OK)
                            G_fatal_error(
                                _("Impossible to read from aux table"));

                        if (UpDate(gradient[0], gradient[1], interpolation,
                                   line_num[i], driver, tab_name) != DB_OK)
                        ...
                        edge =
                            edge_detection(Elaboration, Overlap, parBilin,
                                           *point->x, *point->y, gradient,
                                           alpha, residual, gradHigh, gradLow);
