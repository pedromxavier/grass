...
/* Determine if the string is double, e.g. 123.456, +123.456,
 * -123.456, 1.23456e2 return 1 if double, 0 otherwise */
...
int points_analyse(FILE *ascii_in, FILE *ascii, char *fs, char *td,
                   ...
                   int zcol, int catcol, int region_flag, int ignore_flag)
{
    int i;
    ...
    while (1) {
        ...
        if (ncols > 0 && ntokens != ncols) {
            ...
            if (ignore_flag) {
                G_warning(_("Row %d: '%s' can not be imported into the "
                            "attribute table"),
                          row, buf);
            }
            else {
                G_warning(_("Expected %d columns, found %d columns"), ncols,
                          ntokens);
        ...
        if (xcol >= ntokens || ycol >= ntokens || zcol >= ntokens ||
            ...
            if (ignore_flag) {
                ...
                G_warning(_("ntokens: %d, xcol: %d, ycol: %d, zcol: %d"),
                          ntokens, xcol, ycol, zcol);
                G_fatal_error(_("Broken row %d: '%s'"), row, buf);
            }
        }
        ...
        for (i = 0; i < ntokens; i++) {
            ...
            if ((G_projection() == PROJECTION_LL)) {
                if (i == xcol || i == ycol) {
                    if (i == 0) { /* Save position of original internal token
                                     buffer */
                        /* Prevent memory leaks */
                        sav_buf = tokens[0];
                    }
                    /* check if coordinates are DMS or decimal or not latlong at
                     * all */
                    if (i == xcol) {
                        if (G_scan_easting(tokens[i], &easting, window.proj)) {
                            ...
                            fprintf(stderr, _("Current row %d:\n%s\n"), row,
                                    buf);
                            G_fatal_error(_("Unparsable longitude value in "
                                            "column %d: %s"),
                                          i + 1, tokens[i]);
                        }
                    }
                    ...
                    if (i == ycol) {
                        if (G_scan_northing(tokens[i], &northing,
                            ...
                            fprintf(stderr, _("Current row %d:\n%s\n"), row,
                                    buf);
                            G_fatal_error(
                                _("Unparsable latitude value in column %d: %s"),
                                i + 1, tokens[i]);
                        }
                    }
                } /* if (x or y) */
                ...
                if (strlen(tokens[i]) == 0) {
                    if (i == xcol) {
                        G_fatal_error(
                            _("Unparsable longitude value in column %d: %s"),
                            i + 1, tokens[i]);
                    }
                    if (i == ycol) {
                        G_fatal_error(
                ...
                if (region_flag) {
                    ...
                    /* change to if(>= east,north){skip=1;} to allow correct
                     * tiling */
...
int points_to_bin(FILE *ascii, int rowlen, struct Map_info *Map,
                  dbDriver *driver, char *table, char *fs, char *td, int nrows,
