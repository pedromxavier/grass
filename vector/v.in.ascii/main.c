...
int main(int argc, char *argv[])
{
    ...
    struct Option *old, *new, *delim_opt, *tdelim_opt, *columns_opt, *xcol_opt,
    ...
    desc = NULL;
    G_asprintf(&desc, "point;%s;standard;%s", _("simple x,y[,z] list"),
    ...
    tdelim_opt->description =
        _("Special characters: doublequote, singlequote, none");
    ...
    zcol_opt->description =
        _("First column is 1. If 0, z coordinate is not used");
    ...
    catcol_opt->label = _("Number of column used as category (points mode)");
    catcol_opt->description =
        _("First column is 1. If 0, unique category is assigned to each row "
    ...
    if (format == GV_ASCII_FORMAT_POINT) {
        ...
        points_analyse(ascii, tmpascii, fs, td, &rowlen, &ncols, &minncols,
                       &nrows, &coltype, &colsample, &collen, skip_lines, xcol,
                       ycol, zcol, catcol, region_flag->answer,
                       ignore_flag->answer);
        ...
        if (xcol >= minncols) {
            G_fatal_error(_("'%s' column number > minimum last column number "
                            "(incorrect field separator or format?)"),
                          "x");
        }
        if (ycol >= minncols) {
            G_fatal_error(_("'%s' column number > minimum last column number "
        ...
        if (make_table) {
            ...
            /* this link is added with default 'cat' key, later deleted and
             * replaced by true key name, otherwise if module crashes when the
             * table exists but link is not written it makes troubles */
            ...
            if (columns_opt->answer) {
                ...
                if ((catcol >= 0 && nc != ncols) ||
                    (catcol < 0 && (nc - 1) != ncols)) {
                    G_fatal_error(_("Number of columns defined (%d) does not "
                                    "match number "
                ...
                for (i = 0; i < ncols; i++) {
                    ...
                    case DB_C_TYPE_DOUBLE:
                        if (ctype == DB_C_TYPE_INT) {
                            G_fatal_error(
                                _("Column number %d <%s> defined as integer "
                    ...
                    case DB_C_TYPE_STRING:
                        ...
                        if (length < collen[i]) {
                            G_fatal_error(
                                _("Length of column %d <%s> (%d) is less than "
                                  "maximum value "
                                  "length (%d)"),
                                i + 1, db_get_column_name(column), length,
                                collen[i]);
                        }
                        break;
                    }
        ...
        if (driver) {
            ...
            if (db_close_database_shutdown_driver(driver) == DB_FAILED)
                G_fatal_error(_("Unable to close attribute table. The DBMI "
