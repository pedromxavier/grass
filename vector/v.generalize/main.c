...
int main(int argc, char *argv[])
{
    ...
    loop_support_flag->description =
        _("Do not modify end points of lines forming a closed loop");
    ...
    if (error_out->answer) {
        if (0 > Vect_open_new(&Error, error_out->answer, with_z)) {
            Vect_close(&In);
            G_fatal_error(_("Unable to create error vector map <%s>"),
                          error_out->answer);
        }
    }
        ...
        for (i = 1; i <= n_lines; i++) {
            ...
            for (iter = 0; iter < iterations; iter++) {
                ...
                case SLIDING_AVERAGING:
                    sliding_averaging(Points, slide, look_ahead, loop_support,
                                      with_z);
                    break;
            ...
            if (loop_support == 0) {
                ...
                if (APoints->x[0] != Points->x[0] ||
                    ...
                    G_fatal_error(_("Method '%s' did not preserve first point"),
                                  method_opt->answer);
                ...
                if (Points->x[0] != Points->x[Points->n_points - 1] ||
                    ...
                    G_fatal_error(_("Method '%s' did not preserve loop"),
                                  method_opt->answer);
            }
