...
#define DGL_NODE_SIZEOF_v2(nattr) \
    (sizeof(dglInt32_t) * DGL_IN_SIZE_v2 + (nattr))
#define DGL_NODE_WSIZE_v2(nattr) \
    (DGL_NODE_SIZEOF_v2(nattr) / sizeof(dglInt32_t))
...
#define DGL_EDGESET_WSIZE_v2(C, lattr) \
    (DGL_EDGESET_SIZEOF_v2(C, lattr) / sizeof(dglInt32_t))
#define DGL_EDGESET_ALLOC_v2(C, lattr) (malloc(DGL_EDGESET_SIZEOF_v2(C, lattr)))
#define DGL_EDGESET_REALLOC_v2(P, C, lattr) \
    (realloc(P, DGL_EDGESET_SIZEOF_v2(C, lattr)))

#define DGL_EDGESET_EDGECOUNT_v2(p)     ((p)[DGL_ILA_TOCNT_v2])
#define DGL_EDGESET_EDGEARRAY_PTR_v2(p) ((p) + DGL_ILA_TOARR_v2)
#define DGL_EDGESET_EDGE_PTR_v2(pgrp, p, i) \
...
#define DGL_EDGE_SIZEOF_v2(lattr) \
    (sizeof(dglInt32_t) * DGL_IL_SIZE_v2 + (lattr))
#define DGL_EDGE_WSIZE_v2(lattr) \
    (DGL_EDGE_SIZEOF_v2(lattr) / sizeof(dglInt32_t))
...
#define DGL_FOREACH_NODE_v2(pgrp, pn)                                      \
    for ((pn) = (dglInt32_t *)(pgrp)->pNodeBuffer;                         \
         (pgrp)->pNodeBuffer &&                                            \
         (pn) < (dglInt32_t *)((pgrp)->pNodeBuffer + (pgrp)->iNodeBuffer); \
...
#define DGL_NODEBUFFER_OFFSET_v2(pgrp, p) \
    ((dglInt32_t)((dglByte_t *)p - (dglByte_t *)(pgrp)->pNodeBuffer))

/*
 * Edge Buffer Utilities
 */
#define DGL_EDGEBUFFER_SHIFT_v2(pgrp, o) \
    ((dglInt32_t *)((pgrp)->pEdgeBuffer + (o)))
#define DGL_EDGEBUFFER_OFFSET_v2(pgrp, pl) \
    ((dglInt32_t)((dglByte_t *)pl - (dglByte_t *)(pgrp)->pEdgeBuffer))

int dgl_add_edge_V2(dglGraph_s *pgraph, dglInt32_t nHead, dglInt32_t nTail,
...
int dgl_dijkstra_V2_TREE(dglGraph_s *pgraph, dglSPReport_s **ppReport,
                         dglInt32_t *pDistance, dglInt32_t nStart,
                         dglInt32_t nDestination, dglSPClip_fn fnClip,
                         void *pvClipArg, dglSPCache_s *pCache);
int dgl_dijkstra_V2_FLAT(dglGraph_s *pgraph, dglSPReport_s **ppReport,
                         dglInt32_t *pDistance, dglInt32_t nStart,
                         dglInt32_t nDestination, dglSPClip_fn fnClip,
                         void *pvClipArg, dglSPCache_s *pCache);
int dgl_dijkstra_V2(dglGraph_s *pgraph, dglSPReport_s **ppReport,
                    dglInt32_t *pDistance, dglInt32_t nStart,
                    dglInt32_t nDestination, dglSPClip_fn fnClip,
                    void *pvClipArg, dglSPCache_s *pCache);
...
int dgl_span_depthfirst_spanning_V2_FLAT(dglGraph_s *pgraphIn,
                                         ...
                                         dglSpanClip_fn fnClip,
                                         void *pvClipArg);
int dgl_depthfirst_spanning_V2(dglGraph_s *pgraphIn, dglGraph_s *pgraphOut,
                               dglInt32_t nVertex, void *pvVisited,
...
int dgl_span_minimum_spanning_V2_FLAT(dglGraph_s *pgraphIn,
                                      dglGraph_s *pgraphOut, dglInt32_t nVertex,
                                      dglSpanClip_fn fnClip, void *pvClipArg);
int dgl_minimum_spanning_V2(dglGraph_s *pgraphIn, dglGraph_s *pgraphOut,
                            dglInt32_t nVertex, dglSpanClip_fn fnClip,
                            void *pvClipArg);

int dgl_add_node_V2(dglGraph_s *pgraph, dglInt32_t nId, void *pvNodeAttr,
                    dglInt32_t nFlags);
...
int dgl_edge_t_initialize_V2(dglGraph_s *pGraph, dglEdgeTraverser_s *pTraverser,
