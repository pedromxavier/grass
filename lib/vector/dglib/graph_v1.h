...
#define DGL_NODE_SIZEOF_v1(nattr) \
    (sizeof(dglInt32_t) * DGL_IN_SIZE_v1 + (nattr))
#define DGL_NODE_WSIZE_v1(nattr) \
    (DGL_NODE_SIZEOF_v1(nattr) / sizeof(dglInt32_t))
...
#define DGL_EDGESET_SIZEOF_v1(C, lattr) \
    (sizeof(dglInt32_t) * (DGL_ILA_SIZE_v1) + DGL_EDGE_SIZEOF_v1(lattr) * (C))
#define DGL_EDGESET_WSIZE_v1(C, lattr) \
    (DGL_EDGESET_SIZEOF_v1(C, lattr) / sizeof(dglInt32_t))
#define DGL_EDGESET_ALLOC_v1(C, lattr) (malloc(DGL_EDGESET_SIZEOF_v1(C, lattr)))
#define DGL_EDGESET_REALLOC_v1(P, C, lattr) \
    (realloc(P, DGL_EDGESET_SIZEOF_v1(C, lattr)))
...
#define DGL_EDGE_SIZEOF_v1(lattr) \
    (sizeof(dglInt32_t) * DGL_IL_SIZE_v1 + (lattr))
#define DGL_EDGE_WSIZE_v1(lattr) \
    (DGL_EDGE_SIZEOF_v1(lattr) / sizeof(dglInt32_t))
...
#define DGL_FOREACH_EDGE_v1(pgrp, pla, pl)                        \
    for ((pl) = DGL_EDGESET_EDGEARRAY_PTR_v1(pla);                \
         (pl) < (pla) + DGL_EDGE_WSIZE_v1((pgrp)->EdgeAttrSize) * \
                            DGL_EDGESET_EDGECOUNT_v1(pla);        \
...
#define DGL_NODEBUFFER_OFFSET_v1(pgrp, p) \
    ((dglInt32_t)((dglByte_t *)p - (dglByte_t *)(pgrp)->pNodeBuffer))

/*
 * Edge Buffer Utilities
 */
#define DGL_EDGEBUFFER_SHIFT_v1(pgrp, o) \
    ((dglInt32_t *)((pgrp)->pEdgeBuffer + (o)))
#define DGL_EDGEBUFFER_OFFSET_v1(pgrp, pl) \
    ((dglInt32_t)((dglByte_t *)pl - (dglByte_t *)(pgrp)->pEdgeBuffer))

int dgl_add_edge_V1(dglGraph_s *pgraph, dglInt32_t nHead, dglInt32_t nTail,
...
int dgl_dijkstra_V1_TREE(dglGraph_s *pgraph, dglSPReport_s **ppReport,
                         dglInt32_t *pDistance, dglInt32_t nStart,
                         dglInt32_t nDestination, dglSPClip_fn fnClip,
                         void *pvClipArg, dglSPCache_s *pCache);
int dgl_dijkstra_V1_FLAT(dglGraph_s *pgraph, dglSPReport_s **ppReport,
                         dglInt32_t *pDistance, dglInt32_t nStart,
                         dglInt32_t nDestination, dglSPClip_fn fnClip,
                         void *pvClipArg, dglSPCache_s *pCache);
...
int dgl_add_node_V1(dglGraph_s *pgraph, dglInt32_t nId, void *pvNodeAttr,
                    dglInt32_t nFlags);
...
int dgl_edge_t_initialize_V1(dglGraph_s *pGraph, dglEdgeTraverser_s *pTraverser,
