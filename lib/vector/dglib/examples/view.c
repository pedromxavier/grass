...
static int _print_node(dglGraph_s *pgraph, dglInt32_t *pnode, void *pvarg)
{
    ...
    fprintf(f, "HEAD %-8ld - %-s", dglNodeGet_Id(pgraph, pnode),
            (role > 2)    ? "'H/T'"
            : (role == 2) ? "'T  '"
            : (role == 1) ? "'H  '"
                          : "'A  '");
    ...
    if (role & 1) {
        ...
        for (i = 0, pedge = dglEdgeset_T_First(&edgeaT); pedge;
...
int main(int argc, char **argv)
{
    ...
    fprintf(stdout, "Byte Order: %s\n",
            (graph.Endian == DGL_ENDIAN_LITTLE) ? "Little Endian"
                                                : "Big Endian");
    ...
    fprintf(stdout, "%10ld %10ld %10ld %10ld\n", graph.aOpaqueSet[0],
            graph.aOpaqueSet[1], graph.aOpaqueSet[2], graph.aOpaqueSet[3]);
    fprintf(stdout, "%10ld %10ld %10ld %10ld\n", graph.aOpaqueSet[4],
            graph.aOpaqueSet[5], graph.aOpaqueSet[6], graph.aOpaqueSet[7]);
    fprintf(stdout, "%10ld %10ld %10ld %10ld\n", graph.aOpaqueSet[8],
            graph.aOpaqueSet[9], graph.aOpaqueSet[10], graph.aOpaqueSet[11]);
    fprintf(stdout, "%10ld %10ld %10ld %10ld\n", graph.aOpaqueSet[12],
            graph.aOpaqueSet[13], graph.aOpaqueSet[14], graph.aOpaqueSet[15]);
