...
static int clipper(dglGraph_s *pgraph, dglSPClipInput_s *pIn,
                   dglSPClipOutput_s *pOut, void *pvarg /* caller's pointer */
)
{
...
int main(int argc, char **argv)
{
    ...
    if (fDistance == False) {
        nret = dglShortestPath(&graph, &pReport, from, to, clipper, pclipctx,
                               &spCache);
        ...
        if (nret == 0) {
            ...
            for (i = 0; i < pReport->cArc; i++) {
                printf(
                    "edge[%d]: from %ld to %ld - travel cost %ld - user edgeid "
                    "%ld - distance from start node %ld\n",
                    i, pReport->pArc[i].nFrom, pReport->pArc[i].nTo,
                    dglEdgeGet_Cost(
                        &graph, pReport->pArc[i]
                                    .pnEdge), /* this is the cost from clip() */
        ...
        nret = dglShortestDistance(&graph, &nDistance, from, to, clipper,
                                   pclipctx, &spCache);
