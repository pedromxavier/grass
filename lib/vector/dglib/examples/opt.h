 ...
 *              were taken from the posix getopt() family functions, while our
 *              specific experience in C programming suggested us a proprietary
 *              implementation to make source code more readable at life easier
                (I hope).
 *                      Option format:
 *
 *                      syntax                          name
 ...
 *
 *                      GNO_BEGIN
 *                      GNO_SWITCH( "h", "help", False , & fHelp ,
                            "Print this help." )
 *                      GNO_SWITCH( "t", "tcp", False , & fTcp , NULL )
 *                      GNO_SWITCH( "u", "udp", False , & fUdp , NULL )
 *                      GNO_OPTION( "p", "protocol", "tcp" , & pszProtocol
                            , NULL )
 *                      GNO_OPTION( "i", "interface", "eth0" , & pszInterface
                            , NULL )
 *                      GNO_END
 *
 *
 ...
 *                      all binary-zero fields), and to pass it to GnoParse()
 *together with the typical argc,argv couple of main() args. The array's entries
 *are looked-up and filled with coherent argc,argv values. Some fields are
 *statically filled by user while other are returned by the GnoParse() function.
 *                      Fields set by the user:
 *
 *                      nFlg      = So far the only supported flag is
 *GNO_FLG_SWITCH to address a boolean option type. fDef      = The default value
 *for a boolean option. pszDef    = The default value for a parametric option.
 ...
 *                      the automatic-variable declaration in the main()
 *function.
 *
 *--------------------------------------------------------------------*/
 ...
 *                      I:      ppszv   ->      user-addressed return variable
 *pointer
 *
 *--------------------------------------------------------------------*/
 ...
 *                      I:      pfv             ->      user-addressed return
 *variable pointer
 *
 *--------------------------------------------------------------------*/
#define GNO_SWITCH(pszsopt, pszlopt, fdef, pfv, pszdescr) \
    {0, GNO_FLG_SWITCH, fdef, NULL, pszsopt, pszlopt, pfv, NULL, pszdescr},
