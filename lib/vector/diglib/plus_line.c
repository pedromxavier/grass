...
static int add_line(struct Plus_head *plus, int lineid, int type,
                    const struct line_pnts *Points, const struct bound_box *box,
                    off_t offset)
{
 ...
 * \return line id
 */
int dig_add_line(struct Plus_head *plus, int type,
                 const struct line_pnts *Points, const struct bound_box *box,
                 off_t offset)
{
    int ret;
 ...
 * \return line id
 */
int dig_restore_line(struct Plus_head *plus, int lineid, int type,
 ...
 *
 * Doesn't update area/isle references (dig_del_area() or dig_del_isle()) must
 * be run before the line is deleted if the line is part of such structure).
 * Update is info about line in nodes. If this line is last in node then node is
 * deleted.
 *
 * \param[in,out] plus pointer to Plus_head structure
...
int dig_del_line(struct Plus_head *plus, int line, double x, double y, double z)
{
    ...
    if (i == Node->n_lines) {
        G_fatal_error(
            _("Attempt to delete not registered line %d from node %d"), line,
            N1);
    }
 ...
 * \return 1
 */
int dig_line_set_area(struct Plus_head *plus, plus_t line, int side,
                      plus_t area)
{
    struct P_line *Line;
    struct P_topo_b *topo;
 ...
 */
/*
   int dig_line_set_box(struct Plus_head *plus, plus_t line, struct bound_box *
   Box)
   {
   struct P_line *Line;
 ...
 */
/*
   int dig_line_get_box(struct Plus_head *plus, plus_t line, struct bound_box *
   Box)
   {
   struct P_line *Line;
