    ...
    if (n->level > 0)
        for (i = 0; i < NODECARD; i++) {
            if (n->branch[i].child.pos >= 0) {
                fprintf(fp, "  Branch %d", i);
                rtree_dump_branch_file(fp, &(n->branch[i]), with_z, n->level,
                                       t);
            }
        }
    else
    ...
    while (top >= 0) {
        ...
        if (writeout) {
            ...
            if (top >= 0) {
                ...
                nextfreepos +=
                    (s[top + 1].sn->level ? sidx_nodesize : sidx_leafsize);
            }
        }
    }
                                   ...
                                   struct RTree *t, int off_t_size)
{
    ...
    if (!s) {
        ...
        for (i = 0; i < MAXLEVEL; i++) {
            ...
            for (j = 0; j < MAXCARD; j++) {
                s[i].sn.branch[j].rect.boundary =
                    G_malloc(6 * sizeof(RectReal));
            }
        }
    }
                            ...
                            struct RTree *t, struct Plus_head *Plus)
{
    ...
    for (i = 0; i < maxcard; i++) {
        dig__fread_port_D(nb->n.branch[i].rect.boundary, NUMSIDES, file);
        dig__fread_port_O(&pos, 1, file, Plus->spidx_port.off_t_size);
        /* leaf node: vector object IDs are stored in child.id */
        if (nb->n.level == 0) {
