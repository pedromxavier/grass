...
int Rast3d_key_get_int(struct Key_Value *keys, const char *key, int *i)
{
    ...
    if ((str = G_find_key_value(key, keys)) == NULL) {
        Rast3d_error(
            "Rast3d_key_get_int: cannot find field %s in key structure", key);
        return 0;
    }
    ...
    Rast3d_error("Rast3d_key_get_int: invalid value: field %s in key structure",
                 key);
    return 0;
}
...
int Rast3d_key_get_double(struct Key_Value *keys, const char *key, double *d)
{
    ...
    Rast3d_error(
        "Rast3d_key_get_double: invalid value: field %s in key structure", key);
    return 0;
}

/*---------------------------------------------------------------------------*/

int Rast3d_key_get_string(struct Key_Value *keys, const char *key,
                          char **returnStr)
{
    const char *str;
...
int Rast3d_key_get_value(struct Key_Value *keys, const char *key, char *val1,
                         char *val2, int result1, int result2, int *resultVar)
{
    ...
    if ((str = G_find_key_value(key, keys)) == NULL) {
        Rast3d_error(
            "Rast3d_key_get_value: cannot find field %s in key structure", key);
        return 0;
    }
    ...
    Rast3d_error(
        "Rast3d_key_get_value: invalid type: field %s in key structure", key);
    return 0;
}
