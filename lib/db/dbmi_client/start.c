...
dbDriver *db_start_driver(const char *name)
{
     ...
     * Even if GISRC is set, application may change some variables during
     * runtime, if for example reads data from different gdatabase, location or
     * mapset */

    /* setenv() is not portable, putenv() is POSIX, putenv() in glibc 2.0-2.1.1
     * doesn't conform to SUSv2, G_putenv() as well, but that is what we want,
...
#define pipe(fds) _pipe(fds, 250000, _O_BINARY | _O_NOINHERIT)
    ...
    pid =
        G_spawn_ex(startup, SF_BACKGROUND, SF_REDIRECT_DESCRIPTOR, 0, p1[READ],
                   SF_CLOSE_DESCRIPTOR, p1[WRITE], SF_REDIRECT_DESCRIPTOR, 1,
                   p2[WRITE], SF_CLOSE_DESCRIPTOR, p2[READ], startup, NULL);
