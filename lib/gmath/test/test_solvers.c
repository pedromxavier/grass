...
int test_solvers(void)
{
    ...
    G_math_d_asum_norm(les->x, &val, les->rows);
    if ((val - (double)les->rows) > EPSILON_ITER) {
        G_warning("Error in G_math_solver_pcg abs %2.20f != %i", val,
                  les->rows);
        sum++;
    }
    ...
    G_math_d_asum_norm(les->x, &val, les->rows);
    if ((val - (double)les->rows) > EPSILON_DIRECT) {
        G_warning("Error in G_math_solver_solver_cholesky abs %2.20f != %i",
                  val, les->rows);
        sum++;
    }
    G_math_print_les(les);
    ...
    /*cholesky */ G_math_solver_cholesky_sband(les->A, les->x, les->b,
                                               les->rows, les->rows);
    G_math_d_asum_norm(les->x, &val, les->rows);
    if ((val - (double)les->rows) > EPSILON_DIRECT) {
        G_warning(
    ...
    G_math_d_asum_norm(les->x, &val, les->rows);
    if ((val - (double)les->rows) > EPSILON_ITER) {
        G_warning("Error in G_math_solver_cg_sband abs %2.20f != %i", val,
                  les->rows);
        sum++;
    }
    G_math_print_les(les);
