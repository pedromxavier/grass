...
static int draw_line(int, int, int, const struct line_pnts *,
                     const struct line_cats *, const struct color_rgb *,
                     const struct color_rgb *, int, const char *, double, int,
                     int, int, int, double, struct Colors *, dbCatValArray *,
                     struct Colors *, dbCatValArray *, int, dbCatValArray *,
                     int, dbCatValArray *, int, const struct cat_list *,
                     SYMBOL *, RGBA_Color *, RGBA_Color *, RGBA_Color *, int *,
                     int *, int *, int *, int *);
...
int display_lines(struct Map_info *Map, int type, struct cat_list *Clist,
                  ...
                  int sqrt_flag, int id_flag, int cats_color_flag,
                  int default_width, double width_scale, struct Colors *zcolors,
...

{
    ...
    if (id_flag) {
        if (Vect_level(Map) < 2)
            G_fatal_error(
                _("Unable to display features by id, topology not available. "
    ...
    while (TRUE) {
        ...
        draw_line(type, ltype, line, Points, Cats, color, fcolor, chcat,
                  symbol_name, size, sqrt_flag, id_flag, cats_color_flag,
                  default_width, width_scale, zcolors, cvarr_rgb, colors,
                  cvarr_width, nrec_width, cvarr_size, nrec_size, cvarr_rot,
                  nrec_rot, Clist, Symb, line_color, fill_color, primary_color,
                  &n_points, &n_lines, &n_centroids, &n_boundaries, &n_faces);
    }
...
int draw_line(int type, int ltype, int line, const struct line_pnts *Points,
              const struct line_cats *Cats, const struct color_rgb *color,
              const struct color_rgb *fcolor, int chcat,
              const char *symbol_name, double size, int sqrt_flag, int id_flag,
              int cats_color_flag, int default_width, double width_scale,
              struct Colors *zcolors, dbCatValArray *cvarr_rgb,
              struct Colors *colors, dbCatValArray *cvarr_width, int nrec_width,
              dbCatValArray *cvarr_size, int nrec_size,
              dbCatValArray *cvarr_rot, int nrec_rot,
              ...
              int *n_centroids, int *n_boundaries, int *n_faces)
{
    ...
    if (colors || cvarr_rgb || nrec_width > 0 || nrec_size > 0 || nrec_rot > 0)
        /* only first category */
        Vect_cat_get(Cats,
                     (Clist->field > 0
                          ? Clist->field
    ...
    if (colors || cvarr_rgb) {
        custom_rgb =
            get_table_color(cat, line, colors, cvarr_rgb, &red, &grn, &blu);
    }

    /* random colors */
    if (cats_color_flag) {
        custom_rgb = get_cat_color(line, Cats, Clist, &red, &grn, &blu);
    }

    /* line width */
    if (nrec_width) {
        width = (int)get_property(cat, line, cvarr_width, (double)width_scale,
    ...
    if ((ltype & GV_POINTS) && (Symb != NULL || (nrec_size || nrec_rot))) {
        ...
        if (nrec_size || nrec_rot) {
             ...
             */
            if (Symb) /* unclean free() on first iteration if variables are not
                         init'd to NULL? */
                G_free(Symb);
            Symb = S_read(symbol_name);
            if (Symb == NULL)
                G_warning(
                    _("Unable to read symbol <%s>, unable to display points"),
                    symbol_name);
            else
