...
static int process_line(int, const struct line_pnts *, const struct line_cats *,
                        LATTR *, int, const struct cat_list *);

int display_label(struct Map_info *Map, int type, struct cat_list *Clist,
...
int process_line(int ltype, const struct line_pnts *Points,
                 const struct line_cats *Cats, LATTR *lattr, int chcat,
...
void show_label(double *px, double *py, LATTR *lattr, const char *text)
{
    ...
    if (lattr->has_bgcolor || lattr->has_bcolor) {
        ...
        if (lattr->has_bcolor) {
            D_RGB_color(lattr->bcolor.R, lattr->bcolor.G, lattr->bcolor.B);
