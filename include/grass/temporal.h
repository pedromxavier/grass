...
/*!Spatio temporal extent as double values
   ...
   If no end time is present, because its a time instance,
   then has_end must be 0.
...
/*!
   ...
   This structure is used to store lists of dataset (space time datasets or time
   stamped maps) using the tgisDataset structure internally.
 */
...
typedef struct _tgisDataset {
    ...
    char temporal_type;     /* The temporal type of this dataset:
                               TGIS_ABSOLUTE_TIME,     TGIS_RELATIVE_TIME */
    struct TimeStamp ts;    /* The timestamp of this dataset */
    tgisExtent extent;      /* This is the spatio-temporal extent represented as
                               double values */

    void *metadata; /* A pointer to the dataset specific metadata (not used yet)
                     */

    char dataset_type; /* The type of the dataset: TGIS_RASTER_MAP,
                          TGIS_RASTER3D_MAP, TGIS_VECTOR_MAP, TGIS_STRDS,
                          TGIS_STR3DS, TGIS_STVDS */
...
void tgis_dataset_list_insert(tgisDatasetList *list, char *name, char *mapset,
                              char *creator, DateTime *creation_time,
                              char temporal_type, struct TimeStamp *ts,
                              tgisExtent *extent, void *metadata,
...
int tgis_create_stds(char *stds_name, char stds_type, char temporal_type,
                     char *title, char *description, char *semantic_type,
...
int tgis_modify_stds(char *stds_name, char stds_type, char *title,
                     char *description, char *semantic_type,
                     char *aggregation_type);
...
/* register.c */
/*! Register a map in the temporal database and optionally in a space time
 * dataset */
int tgis_register_map(tgisMap *map, char map_type, char *stds_name);

/*! Unregister a map from the temporal database or optionally from a space time
 * dataset */
int tgis_unregister_map(tgisMap *map, char map_type, char *stds_name);

/*! Register maps in the temporal database and optionally in a space time
 * dataset */
int tgis_register_maps(tgisMapList *list, char map_type, char *stds_name);

/*! Unregister maps from the temporal database or optionally from a space time
 * dataset */
...
/*! Get the information about a specific space time dataset from the temporal
 * database */
