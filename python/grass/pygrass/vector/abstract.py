...
class Info:
    ...
    mapset = property(
        fget=_get_mapset, fset=_set_mapset, doc="Set or obtain the Vector mapset"
    )
    ...
    person = property(
        fget=_get_person, fset=_set_person, doc="Set or obtain the Vector author"
    )
    ...
    title = property(
        fget=_get_title, fset=_set_title, doc="Set or obtain the Vector title"
    )
    ...
    map_date = property(
        fget=_get_map_date, fset=_set_map_date, doc="Set or obtain the Vector map date"
    )
    ...
    scale = property(
        fget=_get_scale, fset=_set_scale, doc="Set or obtain the Vector scale"
    )
    ...
    comment = property(
        fget=_get_comment, fset=_set_comment, doc="Set or obtain the Vector comment"
    )
    ...
    zone = property(
        fget=_get_zone, fset=_set_zone, doc="Set or obtain the Vector projection zone"
    )
    ...
    proj = property(
        fget=_get_proj, fset=_set_proj, doc="Set or obtain the Vector projection code"
    )
    ...
    thresh = property(
        fget=_get_thresh, fset=_set_thresh, doc="Set or obtain the Vector threshold"
    )
    ...
    def open(
        ...
        if self.exist() and self.mode in ("r", "rw"):
            ...
            if self.mode == "r":
                openvect = libvect.Vect_open_old2(
                    self.c_mapinfo, self.name, self.mapset, str(layer)
                )
            # open in READ and WRITE mode
            elif self.mode == "rw":
                openvect = libvect.Vect_open_update2(
                    self.c_mapinfo, self.name, self.mapset, str(layer)
                )
        ...
        self.find = {
            "by_point": PointFinder(self.c_mapinfo, self.table, self.writeable),
            "by_bbox": BboxFinder(self.c_mapinfo, self.table, self.writeable),
            "by_polygon": PolygonFinder(self.c_mapinfo, self.table, self.writeable),
        }
