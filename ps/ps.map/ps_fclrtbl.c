...
int PS_fcolortable(void)
{
    ...
    if (dmin == dmax) { /* if step==0 all sorts of infinite loops and DIV by 0
                           errors follow */
        G_warning(
            _("A floating point colortable must contain a range of values"));
        return 1;
    }
    ...
    if (horiz) {
        ...
        if (ct.tickbar) /* this is the switch to draw tic all the way through
                           bar */
            y1 = t + height;
        else
            y1 = t;
    ...
    /* do nice label: we need so many decimal places to hold all step decimal
     * digits */
    if (step > 100) { /* nice steps do not have > 2 digits, important separate,
                         otherwise */
    ...
    if (strlen(units)) {
        ...
        /* Hint from Glynn:
           You can use the `stringwidth` operator to obtain the width of a
           string (the distance that the current point will move if you `show`
           the string). If the string is short, you can obtain the bounding box
           with a combination of the `charpath` and `pathbbox` operators (if the
           string is long, `charpath` may overflow the maximum path length). */
