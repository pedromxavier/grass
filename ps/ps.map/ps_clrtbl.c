...
#define NSTEPS                                    \
    5 /* number of steps to divide color box when \
          showing color for category data range */
#define FONTFIT_FACT                                                          \
    4.0                  /* how aggressive to be with shrinking the font size \
...
int PS_colortable(void)
{
    ...
    G_debug(3,
            "clrtbl: adjusted ct.x=[%.3f] ct.y=[%.3f] ct.width=[%.3f] "
            "col_width=[%.3f]",
            ct.x, ct.y, ct.width, col_width);
        ...
        for (j = 0; j < ct.cols; j++) {
            ...
            if (!i || dmax == dmin) {
                ...
                if (do_color)
                    ...
                    grey_color_val =
                        (.3 * (double)R + .59 * (double)G + .11 * (double)B) /
                        255.;
                ...
                /* grey border        fprintf(PS.fp, "B F .247 .247 .247 C 1 W
                 * stroke\n"); */
                fprintf(PS.fp, "B F ");
