...
int PS_vpoints_plot(struct Map_info *P_map, int vec)
{
    ...
    if (vector.layer[vec].cats != NULL || vector.layer[vec].where != NULL) {
        ...
        if (vector.layer[vec].cats != NULL) {
            ret = Vect_set_varray_from_cat_string(
                P_map, vector.layer[vec].field, vector.layer[vec].cats,
    ...
    for (line = 1; line <= nlines; line++) {
        ...
        if (vector.layer[vec].sizecol == NULL)
            ...
            if (cvarr_size.ctype == DB_C_TYPE_INT) {
                ret = db_CatValArray_get_value_int(&cvarr_size, cat,
                                                   &size_val_int);
                if (ret != DB_OK) {
            ...
            if (cvarr_size.ctype == DB_C_TYPE_DOUBLE) {
                ret = db_CatValArray_get_value_double(&cvarr_size, cat,
                                                      &size_val);
                if (ret != DB_OK) {
                    G_warning(_("No record for category [%d]"), cat);
                    continue;
            ...
            if (size_val < 0.0) {
                G_warning(
                    _("Attribute is of invalid size [%.3f] for category [%d]"),
                    size_val, cat);
                continue;
            }
        ...
        if (vector.layer[vec].rgbcol != NULL) {
            ...
            if (ret != DB_OK) {
                ...
                if (rgbstring == NULL ||
                    ...
                    G_warning(_("Invalid RGB color definition in column <%s> "
                                "for category [%d]"),
        ...
        if (vector.layer[vec].rotcol == NULL)
            ...
            if (cvarr_rot.ctype == DB_C_TYPE_INT) {
                ret =
                    db_CatValArray_get_value_int(&cvarr_rot, cat, &rot_val_int);
                if (ret != DB_OK) {
                    G_warning(_("No record for category [%d]"), cat);
                    continue;
                }
