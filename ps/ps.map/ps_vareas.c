                              ...
                              dbCatValArray *cvarr_rgb, PSCOLOR *color)
{
    ...
    if (ret != DB_OK) {
        ...
        if (rgbstring == NULL ||
            ...
            G_warning(_("Invalid RGB color definition in column <%s> for "
                        "category [%d]"),
    ...
    if (vector.layer[vec].cats != NULL || vector.layer[vec].where != NULL) {
        ...
        if (vector.layer[vec].cats != NULL) {
            ret = Vect_set_varray_from_cat_string(
                P_map, vector.layer[vec].field, vector.layer[vec].cats, GV_AREA,
                1, Varray);
        }
        else {
    ...
    for (area = 1; area <= na; area++) {
        ...
        if (vector.layer[vec].pat != NULL ||
            ...
            if (vector.layer[vec].rgbcol != NULL) {
                ...
                if (get_ps_color_rgbcol_varea(P_map, vec, area, &cvarr_rgb,
                                              &color) != 1)
                    return 0;
            }
            else {
