...
class VDigitSettingsDialog(wx.Dialog):
    def __init__(
        ...
        btnSave.SetToolTip(_("Close dialog and save changes to user settings file"))
        ...
        for label, key in self._symbologyData():
            ...
            color = csel.ColourSelect(
                ...
                colour=UserSettings.Get(
                    group="vdigit", key="symbol", subkey=[key, "color"]
                ),
                size=(40, 25),
            )
            isEnabled = UserSettings.Get(
            ...
            flexSizer.Add(
                enabled, proportion=0, flag=wx.ALIGN_CENTER | wx.FIXED_MINSIZE
            )
    ...
    def _createGeneralPage(self, notebook):
        ...
        # display section
        #
        box = StaticBox(parent=panel, id=wx.ID_ANY, label=" %s " % _("Display"))
        ...
        flexSizer.Add(
            self.lineWidthValue, proportion=0, flag=wx.ALIGN_CENTER | wx.FIXED_MINSIZE
        )
        flexSizer.Add(
            units,
            proportion=0,
        ...
        # snapping section
        #
        box = StaticBox(parent=panel, id=wx.ID_ANY, label=" %s " % _("Snapping"))
        sizer = wx.StaticBoxSizer(box, wx.VERTICAL)
        flexSizer = wx.FlexGridSizer(cols=3, hgap=5, vgap=5)
        flexSizer.AddGrowableCol(0)
        ...
        flexSizer.Add(text, proportion=0, flag=wx.ALIGN_CENTER_VERTICAL)
        flexSizer.Add(
            self.snappingValue, proportion=0, flag=wx.ALIGN_CENTER | wx.FIXED_MINSIZE
        )
        flexSizer.Add(
            self.snappingUnit, proportion=0, flag=wx.ALIGN_RIGHT | wx.FIXED_MINSIZE
        )

        vertexSizer = wx.BoxSizer(wx.VERTICAL)
        self.snapVertex = CheckBox(
            parent=panel, id=wx.ID_ANY, label=_("Snap also to vertex")
        )
        self.snapVertex.SetValue(
            UserSettings.Get(group="vdigit", key="snapToVertex", subkey="enabled")
        )
        ...
        self.snappingInfo = StaticText(
            ...
            % {"value": self.digit.GetDisplay().GetThreshold(), "units": self.mapUnits},
        )
        vertexSizer.Add(
            self.snappingInfo, proportion=0, flag=wx.ALL | wx.EXPAND, border=1
        )
        ...
        #
        box = StaticBox(
            parent=panel, id=wx.ID_ANY, label=" %s " % _("Select vector features")
        )
        # feature type
        sizer = wx.StaticBoxSizer(box, wx.VERTICAL)
        ...
        for feature in ("point", "line", "centroid", "boundary"):
            ...
            chkbox.SetValue(
                UserSettings.Get(
                    group="vdigit", key="selectType", subkey=[feature, "enabled"]
                )
            )
            inSizer.Add(chkbox, proportion=0, flag=wx.EXPAND | wx.ALL, border=5)
        sizer.Add(inSizer, proportion=0, flag=wx.EXPAND)
        # threshold
        flexSizer = wx.FlexGridSizer(cols=3, hgap=5, vgap=5)
        flexSizer.AddGrowableCol(0)
        text = StaticText(parent=panel, id=wx.ID_ANY, label=_("Select threshold"))
        ...
        self.checkForDupl = CheckBox(
            parent=panel, id=wx.ID_ANY, label=_("Check for duplicates")
        )
        ...
        #
        box = StaticBox(
            parent=panel, id=wx.ID_ANY, label=" %s " % _("Digitize lines/boundaries")
        )
        sizer = wx.StaticBoxSizer(box, wx.VERTICAL)

        self.intersect = CheckBox(parent=panel, label=_("Break lines at intersection"))
        self.intersect.SetValue(
            UserSettings.Get(group="vdigit", key="breakLines", subkey="enabled")
        )

        sizer.Add(self.intersect, proportion=0, flag=wx.ALL | wx.EXPAND, border=1)
        ...
        # digitize areas box
        #
        box = StaticBox(parent=panel, id=wx.ID_ANY, label=" %s " % _("Digitize areas"))
        sizer = wx.StaticBoxSizer(box, wx.VERTICAL)
        ...
        sizer.Add(self.closeBoundary, proportion=0, flag=wx.ALL | wx.EXPAND, border=1)
        ...
        # save-on-exit box
        #
        box = StaticBox(parent=panel, id=wx.ID_ANY, label=" %s " % _("Save changes"))
    ...
    def _createQueryPage(self, notebook):
        ...
        #
        box = StaticBox(
            parent=panel, id=wx.ID_ANY, label=" %s " % _("Choose query tool")
        )
        sizer = wx.StaticBoxSizer(box, wx.VERTICAL)

        LocUnits = self.parent.MapWindow.Map.GetProjInfo()["units"]

        self.queryBox = CheckBox(parent=panel, id=wx.ID_ANY, label=_("Select by box"))
        self.queryBox.SetValue(
            UserSettings.Get(group="vdigit", key="query", subkey="box")
        )

        sizer.Add(self.queryBox, proportion=0, flag=wx.ALL | wx.EXPAND, border=1)
        sizer.Add((0, 5))
        ...
        sizer.Add(self.queryLength, proportion=0, flag=wx.ALL | wx.EXPAND, border=1)
        ...
        self.queryLengthSL = wx.Choice(
            parent=panel, id=wx.ID_ANY, choices=[_("shorter than"), _("longer than")]
        )
        ...
        flexSizer.Add(
            self.queryLengthSL, proportion=0, flag=wx.ALIGN_CENTER | wx.FIXED_MINSIZE
        )
        flexSizer.Add(
            self.queryLengthValue, proportion=0, flag=wx.ALIGN_CENTER | wx.FIXED_MINSIZE
        )
        ...
        sizer.Add(self.queryDangle, proportion=0, flag=wx.ALL | wx.EXPAND, border=1)
        flexSizer = wx.FlexGridSizer(cols=4, hgap=5, vgap=5)
        flexSizer.AddGrowableCol(0)
        ...
        flexSizer.Add(txt, proportion=0, flag=wx.ALIGN_CENTER_VERTICAL)
        flexSizer.Add(
            self.queryDangleSL, proportion=0, flag=wx.ALIGN_CENTER | wx.FIXED_MINSIZE
        )
        flexSizer.Add(
            self.queryDangleValue, proportion=0, flag=wx.ALIGN_CENTER | wx.FIXED_MINSIZE
        )
        flexSizer.Add(units, proportion=0, flag=wx.ALIGN_CENTER_VERTICAL)
        sizer.Add(flexSizer, proportion=0, flag=wx.ALL | wx.EXPAND, border=1)
    ...
    def _createAttributesPage(self, notebook):
        ...
        #
        box = StaticBox(
            parent=panel, id=wx.ID_ANY, label=" %s " % _("Digitize new feature")
        )
        sizer = wx.StaticBoxSizer(box, wx.VERTICAL)

        # checkbox
        self.addRecord = CheckBox(
            parent=panel, id=wx.ID_ANY, label=_("Add new record into table")
        )
        self.addRecord.SetValue(
            UserSettings.Get(group="vdigit", key="addRecord", subkey="enabled")
        )
        sizer.Add(self.addRecord, proportion=0, flag=wx.ALL | wx.EXPAND, border=1)
        # settings
        ...
        #
        box = StaticBox(
            parent=panel, id=wx.ID_ANY, label=" %s " % _("Delete existing feature(s)")
        )
        sizer = wx.StaticBoxSizer(box, wx.VERTICAL)

        # checkbox
        self.deleteRecord = CheckBox(
            parent=panel, id=wx.ID_ANY, label=_("Delete record from table")
        )
        self.deleteRecord.SetValue(
            UserSettings.Get(group="vdigit", key="delRecord", subkey="enabled")
        )
        sizer.Add(self.deleteRecord, proportion=0, flag=wx.ALL | wx.EXPAND, border=1)
        border.Add(
            sizer,
            proportion=0,
            flag=wx.LEFT | wx.RIGHT | wx.BOTTOM | wx.EXPAND,
        ...
        #
        box = StaticBox(
            parent=panel, id=wx.ID_ANY, label=" %s " % _("Geometry attributes")
        )
        sizer = wx.StaticBoxSizer(box, wx.VERTICAL)
        gridSizer = wx.GridBagSizer(hgap=3, vgap=3)
        self.geomAttrb = {
            "length": {"label": _("length")},
            "area": {"label": _("area")},
        ...
        for attrb in ["length", "area", "perimeter"]:
            ...
            win_units = wx.Choice(
                parent=panel, id=wx.ID_ANY, choices=choices, size=(120, -1)
            )
            ...
            if (
                ...
                and attrb in tree.GetLayerInfo(item, key="vdigit")["geomAttr"]
            ):
                check.SetValue(True)
        ...
        gridSizer.Add(
            StaticText(parent=panel, id=wx.ID_ANY, label=note), pos=(3, 0), span=(1, 3)
        )
    ...
    def OnChangeCategoryMode(self, event):
        ...
        UserSettings.Set(
            group="vdigit", key="categoryMode", subkey="selection", value=mode
        )
    ...
    def UpdateSettings(self):
        ...
        # query tool
        if self.queryLength.GetValue():
            UserSettings.Set(group="vdigit", key="query", subkey="selection", value=0)
        else:
            UserSettings.Set(group="vdigit", key="query", subkey="type", value=1)
        UserSettings.Set(
            group="vdigit", key="query", subkey="box", value=self.queryBox.IsChecked()
        )
        UserSettings.Set(
            group="vdigit",
            key="queryLength",
