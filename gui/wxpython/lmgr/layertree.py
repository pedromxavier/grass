...
LMIcons = {
    "wmsImport": MetaIcon(img="layer-wms-add", label=_("Import data from WMS server")),
    "layerCmd": MetaIcon(img="layer-command-add", label=_("Add command layer")),
    "quit": MetaIcon(img="quit", label=_("Quit")),
    "layerRaster": MetaIcon(img="raster", label=_("Add raster map layer")),
    "layerRgb": MetaIcon(img="rgb", label=_("Add RGB map layer")),
    "layerHis": MetaIcon(img="his", label=_("Add HIS map layer")),
    "layerShaded": MetaIcon(
        img="shaded-relief", label=_("Add shaded relief map layer")
    ),
    "layerRastarrow": MetaIcon(img="aspect-arrow", label=_("Add raster flow arrows")),
    "layerRastnum": MetaIcon(img="cell-cats", label=_("Add raster cell numbers")),
    "layerVector": MetaIcon(img="vector", label=_("Add vector map layer")),
    "layerThememap": MetaIcon(
        img="vector-thematic", label=_("Add thematic (choropleth) vector map layer")
    ),
    "layerThemechart": MetaIcon(
        img="vector-chart", label=_("Add thematic chart layer")
    ),
    "layerGrid": MetaIcon(img="layer-grid-add", label=_("Add grid layer")),
    "layerGeodesic": MetaIcon(
        img="shortest-distance", label=_("Add geodesic line layer")
    ),
    "layerRhumb": MetaIcon(img="shortest-distance", label=_("Add rhumbline layer")),
    "layerLabels": MetaIcon(img="label", label=_("Add labels")),
    "layerRaster_3d": MetaIcon(
        img="raster3d",
        label=_("Add 3D raster map layer"),
        desc=_("Note that 3D raster data are rendered only in 3D view mode"),
    ),
    "layerWms": MetaIcon(img="wms", label=_("Add WMS layer.")),
    "layerOptions": MetaIcon(img="options", label=_("Set options")),
    "layerEdited": MetaIcon(img="layer-edit", label=_("Editing mode")),
    "layerBgmap": MetaIcon(img="layer-bottom", label=_("Background vector map")),
}
...
class LayerTree(treemixin.DragAndDrop, CT.CustomTreeCtrl):
    ...
    def OnKeyUp(self, event):
        ...
        if key == wx.WXK_DELETE and self.lmgr and not self.GetEditControl():
    ...
    def OnLayerContextMenu(self, event):
        ...
        if ltype not in ("group", "command"):
            if numSelected == 1:
                ...
                item = wx.MenuItem(
                    self.popupMenu, id=self.popupID["properties"], text=_("Properties")
                )
            ...
            if same and ltype in ("raster", "vector", "rgb", "raster_3d"):
                ...
                self.popupMenu.AppendItem(item)
                self.Bind(
                    wx.EVT_MENU, self.mapdisplay.OnZoomToMap, id=self.popupID["zoom"]
                )
                ...
                if ltype and ltype == "raster" and same:
                    self.popupMenu.Append(
                        self.popupID["zoom1"],
                        _("Zoom to selected map(s) (ignore NULLs)"),
                    )
                    self.Bind(
                        wx.EVT_MENU,
                ...
                self.popupMenu.AppendItem(item)
                self.Bind(
                    wx.EVT_MENU, self.OnSetCompRegFromMap, id=self.popupID["region"]
                )
        ...
        if ltype and ltype == "vector" and numSelected == 1:
            ...
            self.Bind(
                ...
                lambda x: self.lmgr.OnMenuCmd(
                    cmd=["v.out.ogr", "input=%s" % mapLayer.GetName()]
                ),
                id=self.popupID["export"],
            )
            ...
            self.popupMenu.Append(self.popupID["export-pg"], _("Export PostGIS"))
            self.Bind(
                wx.EVT_MENU,
                lambda x: self.lmgr.OnMenuCmd(
                    cmd=["v.out.postgis", "input=%s" % mapLayer.GetName()]
                ),
            ...
            self.Bind(
                wx.EVT_MENU,
                lambda x: self.lmgr.OnMenuCmd(
                    cmd=["v.db.select", "map=%s" % mapLayer.GetName()]
                ),
            ...
            item = wx.MenuItem(
                self.popupMenu, id=self.popupID["pack"], text=_("Create pack")
            )
            self.popupMenu.AppendItem(item)
            self.Bind(
                wx.EVT_MENU,
                lambda x: self.lmgr.OnMenuCmd(
                    cmd=["v.pack", "input=%s" % mapLayer.GetName()]
                ),
            ...
            self.popupMenu.Append(
                self.popupID["colori"], _("Set color table interactively")
            )
            self.Bind(wx.EVT_MENU, self.lmgr.OnVectorRules, id=self.popupID["colori"])

            item = wx.MenuItem(
                self.popupMenu, id=self.popupID["attr"], text=_("Show attribute data")
            )
            item.SetBitmap(MetaIcon(img="table").GetBitmap(self.bmpsize))
            self.popupMenu.AppendItem(item)
            self.Bind(
                wx.EVT_MENU, self.lmgr.OnShowAttributeTable, id=self.popupID["attr"]
            )
            ...
            if vdigitLayer is not layer:
                item = wx.MenuItem(
                    self.popupMenu, id=self.popupID["edit"], text=_("Start editing")
                )
                self.Bind(wx.EVT_MENU, self.OnStartEditing, id=self.popupID["edit"])
            else:
                item = wx.MenuItem(
                    self.popupMenu, id=self.popupID["edit"], text=_("Stop editing")
                )
        ...
        # raster layers (specific items)
            ...
            if numSelected == 1:
                item = wx.MenuItem(
                    self.popupMenu, id=self.popupID["export"], text=_("Export")
                )
                item.SetBitmap(MetaIcon(img="layer-export").GetBitmap(self.bmpsize))
                self.popupMenu.AppendItem(item)
                self.Bind(
                    wx.EVT_MENU,
                    lambda x: self.lmgr.OnMenuCmd(
                        cmd=["r.out.gdal", "input=%s" % mapLayer.GetName()]
                    ),
                    id=self.popupID["export"],
                )
                ...
                self.Bind(
                    wx.EVT_MENU,
                    lambda x: self.lmgr.OnMenuCmd(
                        cmd=["r.pack", "input=%s" % mapLayer.GetName()]
                    ),
                    id=self.popupID["pack"],
                )

                lmapset = self.GetLayerInfo(
                    self.layer_selected, key="maplayer"
                ).GetMapset()
                if lmapset != currentMapset:
                    self.popupMenu.Append(
                        self.popupID["copy"], _("Make a copy in the current mapset")
            ...
            if len(selected) < 2:
                ...
                self.Bind(
                    wx.EVT_MENU, self.lmgr.OnRasterRules, id=self.popupID["colori"]
                )

            item = wx.MenuItem(
                self.popupMenu, id=self.popupID["hist"], text=_("Histogram")
            )
            item.SetBitmap(
            ...
            if numSelected == 1:
                item = wx.MenuItem(
                    self.popupMenu, id=self.popupID["prof"], text=_("Profile")
                )
                item.SetBitmap(
            ...
            if numSelected == 1:
                ...
                self.popupMenu.Append(self.popupID["color"], _("Set color table"))
                self.Bind(
                    wx.EVT_MENU, self.OnRasterColorTable, id=self.popupID["color"]
                )
                ...
                self.popupMenu.AppendItem(item)
                self.Bind(
                    wx.EVT_MENU, self.OnUnivariateStats, id=self.popupID["univar"]
                )
        ...
        # web service layers (specific item)
            ...
            self.popupMenu.Append(self.popupID["save_ws"], _("Save web service layer"))
    ...
    def OnTopology(self, event):
        ...
        mapLayer = self.GetLayerInfo(self.layer_selected, key="maplayer")
        cmd = ["v.build", "map=%s" % mapLayer.GetName()]
    ...
    def OnAlignCompRegToRaster(self, event):
        ...
        if len(selected) != 1 or self.GetLayerInfo(selected[0], key="type") != "raster":
            return

        kwargs = {"align": self.GetLayerInfo(selected[0], key="maplayer").GetName()}
    ...
    def OnCopyMap(self, event):
        ...
        self.AddLayer(
            ltype, lname=lnameDst, lcmd=["d.%s" % module, "map=%s" % lnameDst]
        )
    ...
    def ChangeLayerOpacity(self, layer, value):
        ...
        if (
            self.GetMapDisplay().GetToolbar("vdigit")
            and self.GetMapDisplay().GetToolbar("vdigit").GetLayer() == maplayer
        ):
            alpha = int(value * 255)
            self.GetMapDisplay().GetWindow().digit.GetDisplay().UpdateSettings(
                alpha=alpha
            )
        ...
        if element in ("raster", "vector", "raster_3d"):
            ...
            for item in reversed(items):
                if action == "delete":
                    ...
                    self.SetLayerInfo(item, key="cmd", value=cmd)
                    self.ChangeLayer(item)
                    newlabel = self.GetItemText(item).replace(
                        "@" + mapset, "@" + newname
                    )
                    self.SetItemText(item, newlabel)
    ...
    def AddLayer(
        ...
        if ltype == "command":
            ...
            ctrl = GenBitmapButton(self, id=wx.ID_ANY, bitmap=btnbmp, size=(24, 24))
        ...
        if ltype != "group":
            ...
            maplayer = self.Map.AddLayer(
                pos=pos,
                ltype=ltype,
                command=self.GetLayerInfo(prevItem, key="cmd"),
                name=name,
                active=checked,
                hidden=False,
                opacity=lopacity,
    ...
    def PropertiesDialog(self, layer, show=True):
        ...
        ltype = self.GetLayerInfo(layer, key="type")
        if (
            "propwin" in self.GetLayerInfo(layer)
            and self.GetLayerInfo(layer, key="propwin") is not None
        ):
            # recycle GUI dialogs
        ...
        Debug.msg(3, "LayerTree.PropertiesDialog(): ltype=%s" % ltype)
    ...
    def OnDeleteLayer(self, event):
        ...
        if item != self.root:
            Debug.msg(
                3, "LayerTree.OnDeleteLayer(): name=%s" % (self.GetItemText(item))
            )
        else:
            self.root = None
    ...
    def OnLayerChecked(self, event):
        ...
        if not self.first:
            ...
            if self.GetLayerInfo(item, key="type") == "group":
                ...
                if mapLayer and (
                    not digitToolbar
                    or (digitToolbar and digitToolbar.GetLayer() != mapLayer)
                ):
                    # ignore when map layer is edited
                    self.Map.ChangeLayerActive(mapLayer, checked)
                    self._giface.workspaceChanged.emit()
        ...
        # nviz
        if self.mapdisplay.IsPaneShown("3d") and self.GetPyData(item) is not None:
    ...
    def RecreateItem(self, dragItem, dropTarget, parent=None):
        ...
        Debug.msg(4, "LayerTree.RecreateItem(): layer=%s" % self.GetItemText(dragItem))
        ...
        if self.GetLayerInfo(dragItem, key="type") == "command":
            ...
            # recreate data layer
            btnbmp = LMIcons["layerOptions"].GetBitmap((16, 16))
            newctrl = GenBitmapButton(self, id=wx.ID_ANY, bitmap=btnbmp, size=(24, 24))
        ...
        if dropTarget is not None and dropTarget != self.GetRootItem():
            ...
            if self.GetLayerInfo(afteritem, key="type") == "group":
                newItem = self.PrependItem(
                    afteritem, text=text, ct_type=1, wnd=newctrl, image=image, data=data
                )
    ...
    def GetOptData(self, dcmd, layer, params, propwin):
        ...
        if dcmd:
            ...
            if not mapText or not found:
                propwin.Hide()
                GWarning(parent=self, message=_("Map <%s> not found.") % mapName)
                return
                ...
                if len(mapLayer.GetCmd()) > 0:
                    ...
                    nlayers = len(
                        mapWin.Map.GetListOfLayers(
                            ltype=("raster", "raster_3d", "vector"), active=True
                        )
                    )
                    if nlayers < 2:
                        mapWin.ResetView()
    ...
    def GetVisibleLayers(self, skipDigitized=False):
        ...
        Debug.msg(5, "LayerTree.GetVisibleLayers(): items=%s" % (reversed(itemList)))
    ...
    def ChangeLayer(self, item):
        ...
        if self.mapdisplay.GetToolbar("rdigit"):
            ...
            self.mapdisplay.GetToolbar("rdigit").UpdateRasterLayers(rasters)
            self.mapdisplay.GetToolbar("rdigit").SelectDefault()
